...
Downstream adoption of the released kernel surface is underway and has already reached three components. `basis-gateway` v0.2.0 is released and adds an operation-aware enforcement path (`POST /v1/evaluate/operation-aware`, feature-flagged via `OPERATION_AWARE_ENABLED` and disabled by default) alongside its unchanged, always-registered v0.1 path; `basis-gateway`'s Phase 1B (1B.1, 1B.2, and 1B.3) has since merged separately from that release, implementing trusted-ingress mTLS producer authentication and admission for its bounded, approved scope. `basis-console` v0.2.0 is released and consumes that operation-aware path through its Decision Simulator, presenting the identical typed result across its Operator and Training presentation modes. `basis-adapters` v0.2.0 is released and adds adapter-side deterministic evidence-material construction, RFC 8785 canonicalization, and digest generation (ADR-0007 Stage 1) alongside its unchanged nine-protocol normalization surface — adapters still do not authenticate a producer, mint a final evidence reference, submit to the gateway, or execute protocol operations. The remaining gap is not adapter evidence alignment, gateway-side producer authentication, or `basis-producer`'s evidence retention, reference lifecycle, and authenticated gateway client (all implemented) — it is composing those already-implemented pieces with real adapter normalization (Phase 4, REST adapter composition); `basis-producer`'s Phase 2A evidence retention, Phase 2B reference lifecycle, and Phase 3 authenticated gateway client are all implemented and merged. All three releases preserve prior-version compatibility unmodified. Ecosystem-wide operation-aware support is not yet complete: `basis-producer`'s adapter composition (Phase 4), execution-result evidence, `basis-identity` evidence alignment (`basis-identity` is released at `v0.1.0`; evidence alignment against the operation-aware surface has not begun), deployment packaging, and end-to-end validation remain future work — see **Current State** and the Downstream Rollout Sequence in [`ROADMAP.md`](ROADMAP.md), which reflects `basis-adapters` v0.2.0 and the corrected status described in [`docs/architecture/operation-producer-discovery-assessment.md`](docs/architecture/operation-producer-discovery-assessment.md). See [`docs/glossary.md`](docs/glossary.md) and the operation-aware architecture documents linked under **For Architecture Reviewers** below for the conceptual model those contracts publish.
...
- [`docs/architecture/operation-producer-and-execution-boundary.md`](docs/architecture/operation-producer-and-execution-boundary.md) — architecture-planning only; defines the logical roles, trust-establishment rules, request handoff, correlation model, and decision-gated implementation sequence for the gap between `basis-adapters` normalization and `basis-gateway` trusted-producer ingestion, and between gateway enforcement and actual OT protocol execution: `basis-producer`'s Phase 2A evidence retention, Phase 2B reference lifecycle, and Phase 3 authenticated gateway client are all implemented, Phase 4 adapter composition is unimplemented, and the OT protocol-execution boundary is entirely unimplemented
- [`docs/architecture/adapter-evidence-construction-semantics.md`](docs/architecture/adapter-evidence-construction-semantics.md) — accepted architecture specification, adopted by [ADR-0007](docs/adr/0007-adapter-evidence-construction.md); answers the operation-producer-and-execution-boundary document's Stage 2 decision gate by defining adapter evidence material, its fixed RFC 8785 canonicalization requirements, digest semantics, `reference_id` semantics, and construction ownership between `basis-adapters` and `basis-producer`, whose Phase 2A retention foundation, Phase 2B reference assembly, and Phase 3 authenticated gateway client are all implemented; implementation of the remaining adapter-composition phase (Phase 4) remains separate and decision-gated
- [ADR-0008](docs/adr/0008-producer-workload-authentication-and-admission.md) — accepted; `basis-gateway` Phase 1B (1B.1, 1B.2, and 1B.3) is fully merged and complete for its bounded, approved scope, and the broader bounded producer reference slice remains incomplete: the operation-producer runtime now exists (`basis-producer`) and has implemented evidence retention (Phase 2A), reference lifecycle (Phase 2B), and the gateway client (Phase 3); adapter composition (Phase 4) remains unimplemented; answers the operation-producer-and-execution-boundary document's Stage 4 decision gate by selecting mutual TLS as the first normative producer-to-gateway workload-authentication profile, separating authentication from gateway admission and producer identity from authorization subject identity, and authorizing (without itself implementing) a bounded producer reference slice
- [`docs/architecture/operation-producer-discovery-assessment.md`](docs/architecture/operation-producer-discovery-assessment.md) — non-normative discovery assessment; cross-repository, evidence-based inventory of what already exists for the operation-producer role across all inspected implementation repositories, verifying the architecture above against actual code; does not assign ownership or supersede an ADR
- [`docs/architecture/bounded-operation-producer-reference-implementation-plan.md`](docs/architecture/bounded-operation-producer-reference-implementation-plan.md) — implementation planning; translates ADR-0008 and ADR-0007 into a repository-aware, phased implementation sequence for the bounded producer reference slice, now targeting the permanent `basis-producer` component per ADR-0010 — a digest-addressed evidence-retention mechanism with post-retention reference binding, a fixed dual-authentication model (mTLS producer workload plus separate bearer-authenticated authorization subject), admission configuration shape, identifier ownership, layered fail-closed semantics, and test/conformance strategy; Phase 1A is complete (Outcome B), gateway Phase 1B (1B.1, 1B.2, and 1B.3) is fully merged and complete, Phase 2A (evidence-retention foundation), Phase 2B (reference lifecycle), and Phase 3 (producer gateway client) are all implemented and merged, and Phase 4 (REST vertical slice) and later producer phases remain unimplemented — see ADR-0009 below
...
- [`docs/roadmaps/operator-and-training-experience.md`](docs/roadmaps/operator-and-training-experience.md) — the deferred Operator and Training experience doctrine and maturity roadmap for a professional, responsive, investigation-oriented Operator surface with rapid access to trustworthy evidence, deep Training playbooks, and eventual UI/CLI equivalence. Doctrine is durable; broad implementation is deferred until real investigation, trusted-producer, execution-evidence, and identity-telemetry workflows exist
...
- [`docs/architecture/operation-producer-and-execution-boundary.md`](docs/architecture/operation-producer-and-execution-boundary.md) — architecture-planning for the next remaining integration boundary named in `ROADMAP.md` (Phase 4 adapter composition and protocol execution; Phase 2A, Phase 2B, and Phase 3 are all implemented): no implementation work is authorized in `basis-adapters`, `basis-gateway`, `basis-schemas`, `basis-identity`, or `basis-core` by this document alone
...
- [ADR-0010](docs/adr/0010-establish-basis-producer-as-operation-producer-runtime.md) — Accepted; establishes `basis-producer` as the permanent operation-producer-runtime repository and component; the public `basis-foundation/basis-producer` repository now exists, and its first three implementation phases (Phase 2A — evidence-retention foundation; Phase 2B — reference lifecycle; Phase 3 — authenticated gateway client) are all implemented and merged; Phase 4 (REST adapter composition) is next and has not yet begun
