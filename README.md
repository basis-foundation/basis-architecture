...
- [`docs/architecture/operation-aware-evidence-provenance-semantics.md`](docs/architecture/operation-aware-evidence-provenance-semantics.md) — the top-level explanation, per-rule reason/explanation projection, and bundle identity semantics `basis-core` is expected to implement and `basis-schemas` is expected to publish consistently across its canonical compatibility fixtures
