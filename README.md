...
- [`docs/architecture/operation-producer-and-execution-boundary.md`](docs/architecture/operation-producer-and-execution-boundary.md) — architecture-planning only; defines the logical roles, trust-establishment rules, request handoff, correlation model, and decision-gated implementation sequence for the gap between `basis-adapters` normalization and `basis-gateway` trusted-producer ingestion, and between gateway enforcement and actual OT protocol execution: `basis-producer`'s Phase 2A evidence retention and Phase 2B reference lifecycle are both implemented, Phase 3 gateway submission is unimplemented, and the OT protocol-execution boundary is entirely unimplemented
- [`docs/architecture/adapter-evidence-construction-semantics.md`](docs/architecture/adapter-evidence-construction-semantics.md) — accepted architecture specification, adopted by [ADR-0007](docs/adr/0007-adapter-evidence-construction.md); answers the operation-producer-and-execution-boundary document's Stage 2 decision gate by defining adapter evidence material, its fixed RFC 8785 canonicalization requirements, digest semantics, `reference_id` semantics, and construction ownership between `basis-adapters` and `basis-producer`, whose Phase 2A retention foundation and Phase 2B reference assembly are both implemented; implementation of the remaining gateway-submission phase (Phase 3) remains separate and decision-gated
- [ADR-0008](docs/adr/0008-producer-workload-authentication-and-admission.md) — accepted; `basis-gateway` Phase 1B (1B.1, 1B.2, and 1B.3) is fully merged and complete for its bounded, approved scope, and the broader bounded producer reference slice remains incomplete: the operation-producer runtime now exists (`basis-producer`) and has implemented both evidence retention (Phase 2A) and reference lifecycle (Phase 2B), but the gateway client (Phase 3) remains unimplemented; answers the operation-producer-and-execution-boundary document's Stage 4 decision gate by selecting mutual TLS as the first normative producer-to-gateway workload-authentication profile, separating authentication from gateway admission and producer identity from authorization subject identity, and authorizing (without itself implementing) a bounded producer reference slice
...
- [`docs/architecture/operation-producer-and-execution-boundary.md`](docs/architecture/operation-producer-and-execution-boundary.md) — architecture-planning for the next remaining integration boundary named in `ROADMAP.md` (Phase 3 gateway submission and protocol execution; Phase 2B reference lifecycle is implemented): no implementation work is authorized in `basis-adapters`, `basis-gateway`, `basis-schemas`, `basis-identity`, or `basis-core` by this document alone
