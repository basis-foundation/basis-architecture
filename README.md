...
- [ADR-0008](docs/adr/0008-producer-workload-authentication-and-admission.md) — accepted; implementation has not yet begun; answers the operation-producer-and-execution-boundary document's Stage 4 decision gate by selecting mutual TLS as the first normative producer-to-gateway workload-authentication profile, separating authentication from gateway admission and producer identity from authorization subject identity, and authorizing (without implementing) a bounded producer reference slice
...
- [ADR-0009](docs/adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — proposed trusted mTLS ingress and gateway certificate-handoff topology; no implementation work in `basis-gateway` or any other repository is authorized until this ADR is formally accepted
