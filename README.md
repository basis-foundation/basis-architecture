...
Downstream adoption of the released kernel surface is underway and has already reached two components. `basis-gateway` v0.2.0 is released and adds an operation-aware enforcement path (`POST /v1/evaluate/operation-aware`, feature-flagged via `OPERATION_AWARE_ENABLED` and disabled by default) alongside its unchanged, always-registered v0.1 path. `basis-console` v0.2.0 is released and consumes that operation-aware path through its Decision Simulator, presenting the identical typed result across its Operator and Training presentation modes. Both releases preserve `v0.1.0`/`v0.1.1` compatibility unmodified. Ecosystem-wide operation-aware support is not yet complete: trusted operation-producer identity, `basis-adapters` context and evidence alignment (`basis-adapters` remains at `v0.1.0`), execution-result evidence, `basis-identity` evidence alignment (`basis-identity` is released at `v0.1.0`; evidence alignment against the operation-aware surface has not begun), deployment packaging, and end-to-end validation remain future work — see **Current State** and the Downstream Rollout Sequence in [`ROADMAP.md`](ROADMAP.md). See [`docs/glossary.md`](docs/glossary.md) and the operation-aware architecture documents linked under **For Architecture Reviewers** below for the conceptual model those contracts publish.
