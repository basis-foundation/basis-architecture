...
- [ADR-0009](docs/adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — proposed, not yet accepted; answers the bounded implementation plan's Phase 1A gate after `basis-gateway`'s spike found direct application-level mTLS termination not viable (Outcome B); selects a trusted NGINX mTLS ingress, a Unix-domain-socket backend channel, and a certificate-handoff contract that preserves ADR-0008's gateway-owned URI SAN derivation and producer admission unchanged
