...
operation-producer runtime                         Next remaining integration boundary
        (basis-producer — established by ADR-0010, Accepted, repository not yet created)
        ↓
...
**Permanent operation-producer-runtime repository placement — architecture accepted.** [ADR-0010](docs/adr/0010-establish-basis-producer-as-operation-producer-runtime.md) answers the repository-placement decision gate the boundary document's §11 and the bounded implementation plan's §30 both left open: it establishes `basis-producer` (repository `basis-foundation/basis-producer`, public, Python package `basis_producer`) as the permanent repository and component for the operation-producer runtime, as part of the BASIS Core Services Distribution. ADR-0010 is recorded `Accepted`, following a dedicated formal-acceptance review separate from the PR that merged it as `Proposed` — consistent with this repository's governance convention. Acceptance authorizes creation of the public `basis-producer` repository and its first implementation phase, **Phase 2A — evidence-retention foundation** (canonical evidence bytes, expected SHA-256 digest, digest-addressed durable blob storage, verified retrieval), restating rather than redesigning the bounded plan's own §26 phase sequence. No `basis-producer` repository exists yet; repository creation and Phase 2A implementation are the next steps, not work performed by this acceptance.
