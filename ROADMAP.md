...
- `basis-adapters` exists as a separate public repository; `v0.1.0` and `v0.2.0` are both released, and support nine OT protocols and platforms (REST, BACnet, Modbus, OPC UA, MQTT, DNP3, IEC 61850, KNX, and Niagara), each normalization-complete. `v0.2.0` adds adapter-side deterministic evidence-material construction, RFC 8785 canonicalization, and digest generation (ADR-0007 Stage 1) alongside the unchanged nine-protocol normalization surface. Adapters themselves still do not authenticate a producer, persist evidence, mint a final evidence reference, submit to the gateway, or execute protocol operations. Producer workload credential custody, evidence persistence, final evidence-reference minting, and authenticated gateway submission belong to the operation-producer runtime, `basis-producer`, under ADR-0010's accepted architecture; producer workload authentication and admission remain gateway-owned responsibilities under ADR-0008 and ADR-0009. `basis-producer`'s bounded REST reference implementation (evidence retention, reference lifecycle, authenticated gateway submission, and adapter composition) is now complete. Protocol execution is not among those accepted operation-producer responsibilities; ADR-0011 (`Proposed`, not `Accepted`) proposes colocating a separate, logically distinct protocol-executor role in that same `basis-producer` runtime for the first bounded execution reference topology. See **Next Producer and Execution-Evidence Boundary** below.
...
**The discovery assessment of this boundary is complete, and an execution-model ADR is now the active proposed architecture decision.** [`docs/architecture/execution-boundary-discovery-assessment.md`](docs/architecture/execution-boundary-discovery-assessment.md) inventories, against actual current-state implementation evidence across `basis-producer`, `basis-gateway`, `basis-core`, `basis-adapters`, `basis-schemas`, and `basis-identity`, exactly what exists and what is missing beyond authorization; analyzes ownership and deployment-topology options without presuming any of them (including whether a new repository or process is required); and identifies the decisions this ecosystem must make, and in most cases route through the ADR process, before protocol execution can be implemented. It is non-normative: it does not itself authorize execution implementation, does not select a final execution model, and does not modify `basis-schemas` or any implementation repository. It is merged and complete, and now serves as the evidentiary input to [ADR-0011](docs/adr/0011-protocol-execution-role-and-bounded-reference-topology.md), below, rather than as ongoing work. Architecture work on this boundary should be read starting from that assessment; the paragraphs below record the state as of the reconciliation that preceded it.

**An execution-model ADR is now proposed and is the active architecture decision for this boundary.** [ADR-0011](docs/adr/0011-protocol-execution-role-and-bounded-reference-topology.md) — `Proposed`, not `Accepted` — answers the discovery assessment's own §20 recommendation: it proposes a distinct logical protocol-executor role, architecturally separate from the operation-producer role, and proposes same-process colocation with the existing `basis-producer` reference implementation as the default topology for the first bounded execution reference slice, fixing the invariants execution must preserve regardless of topology. It does not select the authorization-to-execution binding mechanism, does not define an execution-lifecycle vocabulary, does not define execution-evidence schemas, and does not create `basis-executor` or any other new repository — each of those is a follow-on decision gate the ADR itself names, unresolved before any bounded execution implementation may dispatch an operation. This is an architecture decision in progress, not protocol execution underway; formal acceptance of ADR-0011, and satisfaction of its own follow-on gates, both remain outstanding.
