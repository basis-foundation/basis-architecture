...
This roadmap does not define final schemas for this boundary, does not implement adapter behavior, and does not claim execution is currently supported anywhere in the ecosystem. `basis-gateway`'s existing operation-producer trust classification (`OPERATION_PRODUCER_SUBJECT_IDS`) is a configuration-driven subject-ID allowlist for the gateway's own callers; it was, at the time this paragraph was originally written, described as distinct from an as-yet-undefined "adapter-to-gateway trusted-producer contract." That description is no longer current: the producer-authentication mechanism itself is no longer an open question — [ADR-0008](docs/adr/0008-producer-workload-authentication-and-admission.md) selected mutual TLS as the first normative profile, and Phase 1A of the bounded reference implementation plan has since completed (Outcome B), with [ADR-0009](docs/adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) now defining the trusted-ingress topology needed to carry that authenticated certificate to `basis-gateway` — see the two paragraphs below. What remains genuinely open is narrower than "the whole contract": permanent operation-producer-runtime repository ownership, category-scoped producer capabilities, execution architecture, execution evidence, and broader retry/replay/idempotency semantics, none of which this roadmap defines final schemas for or claims are scheduled. The bounded producer-authentication reference slice, by contrast, now has an explicit, ordered implementation sequence (Phase 1A complete; Phase 1B blocked pending ADR-0009's formal acceptance) — it is no longer accurate to say existing architecture lacks enough authority to call that slice ordered, even though the broader producer/execution program referenced under **Identity-to-Operation Contract and Interoperability** and **Post-Authentication Identity Activity, Correlation, and Detection** below remains a longer-range research direction.

**Producer workload authentication and gateway admission — architecture accepted, implementation not yet begun.** [ADR-0008](docs/adr/0008-producer-workload-authentication-and-admission.md) answers the authentication-mechanism decision gate named above: it selects mutual TLS as the first normative producer-to-gateway workload-authentication profile, distinguishes authentication from gateway admission, distinguishes producer workload identity from authorization subject identity, and authorizes — without implementing — a first bounded reference slice (REST adapter, producer-minted `AdapterEvidenceReference`, mTLS submission to `basis-gateway`, stopping at the authorization disposition, no protocol execution). ADR-0008 is recorded `Accepted`, consistent with this repository's ADR governance convention that formal acceptance, not merging or drafting, opens an implementation gate — the producer workload-authentication and admission architecture is accepted; implementation has not yet begun. `basis-gateway`, `basis-identity`, `basis-adapters`, and `basis-core` are unchanged by ADR-0008. No mTLS validation, certificate handling, or gateway admission configuration exists in any repository yet. Implementation planning for the bounded REST-based operation-producer reference slice ADR-0008 authorizes is now defined — see [`docs/architecture/bounded-operation-producer-reference-implementation-plan.md`](docs/architecture/bounded-operation-producer-reference-implementation-plan.md), which selects a provisional (not permanent) implementation location, a five-phase implementation PR sequence across `basis-gateway` and a new, explicitly reference-scoped repository, the minimal digest-addressed evidence-retention and identifier decisions the first implementation PR needs, and a fixed dual-authentication model in which mTLS authenticates the producer workload while the gateway's existing bearer path authenticates the authorization subject. The operation-producer runtime's permanent repository placement remains an explicit open question, deferred to the decision gate the plan's own §30 restates (per the boundary document's §11). Protocol execution remains future work, out of scope for the bounded reference slice ADR-0008 authorizes.

**mTLS termination topology — Phase 1A spike complete (Outcome B); trusted-ingress architecture Proposed, not yet Accepted.** The implementation plan's Phase 1A named the mTLS termination topology — where mTLS terminates and how a validated client certificate's identity reaches gateway application code — as a hard, blocking gate before any producer-admission implementation. `basis-gateway`'s Phase 1A spike (`docs/spikes/producer-mtls-certificate-exposure.md`, merged to `main`) completed that gate and reached **Outcome B: direct application termination is not viable.** Uvicorn can require and validate producer client certificates at the TLS layer, but no documented, stable interface carries the validated peer certificate into ASGI application code under the current Uvicorn/ASGI stack, and the standardized extension that would provide one remains unimplemented upstream. [ADR-0009](docs/adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) and its companion [`docs/architecture/producer-mtls-proxy-trust-boundary.md`](docs/architecture/producer-mtls-proxy-trust-boundary.md) answer this gate: they select a trusted NGINX mTLS ingress terminating producer TLS in front of `basis-gateway`, a Unix-domain-socket backend channel, and a certificate-handoff contract (the authenticated leaf certificate, forwarded in a private internal header the ingress unconditionally overwrites) that preserves ADR-0008's assignment of URI SAN derivation and producer admission entirely to `basis-gateway`. ADR-0009 is recorded `Proposed`; it is not yet `Accepted`. **Phase 1B (gateway producer-admission implementation) remains blocked, now specifically on ADR-0009's formal acceptance** rather than on undecided topology. No implementation of the trusted ingress, the internal certificate header, or gateway admission logic exists in any repository yet; `basis-schemas` is unaffected (no schema change is proposed by ADR-0009).
