...
- The remainder of the operation-producer runtime (gateway submission in Phase 3), `basis-identity` evidence alignment, `basis-deploy` packaging, and end-to-end validation — see the **Downstream Rollout Sequence** below. The `basis-producer` repository exists and its Phase 2A — digest-addressed evidence retention and verified retrieval — and Phase 2B — `reference_id` minting and final `AdapterEvidenceReference` assembly — are both implemented and merged; Phase 3 onward has not begun. The gateway-side authentication boundary the producer runtime will depend on is complete for its bounded scope (`basis-gateway` Phase 1B — 1B.1, 1B.2, 1B.3 — merged), and `basis-adapters`' own evidence-material construction (ADR-0007 Stage 1) is likewise complete, per the bullets above; the bounded producer slice as a whole remains incomplete.
...
basis-producer Phase 2B reference lifecycle        Implemented — merged
        ...
        ↓
basis-producer Phase 3 gateway client              Next — not yet implemented
