...
- Trusted operation-producer identity and integration — gateway-side authentication/admission is implemented and merged; the operation-producer runtime, `basis-producer`, now exists as a repository and has implemented its first three phases. `basis-adapters` `v0.2.0` has already implemented the adapter-owned half of adapter evidence alignment (deterministic evidence-material construction, canonicalization, and digest generation, per ADR-0007 Stage 1); `basis-gateway`'s Phase 1B (1B.1, 1B.2, and 1B.3) has since merged, implementing the trusted-ingress mTLS topology, certificate-derived URI SAN identity, and exact producer admission for the bounded, approved scope. `basis-producer`'s Phase 2A — digest-addressed evidence retention and verified retrieval — Phase 2B — `reference_id` minting, durable reference binding, and final `AdapterEvidenceReference` assembly — and Phase 3 — the authenticated `GatewayClient` submitting against that now-implemented gateway boundary, with independent mTLS producer and bearer authorization-subject credentials — are all implemented and merged. What remains not yet started is composing that authenticated client with real adapter normalization output (Phase 4, REST adapter composition); see [ADR-0010](docs/adr/0010-establish-basis-producer-as-operation-producer-runtime.md), Accepted. See **Next Producer and Execution-Evidence Boundary** below.
...
- The remainder of the operation-producer runtime (adapter composition in Phase 4), `basis-identity` evidence alignment, `basis-deploy` packaging, and end-to-end validation — see the **Downstream Rollout Sequence** below. The `basis-producer` repository exists and its Phase 2A — digest-addressed evidence retention and verified retrieval — Phase 2B — `reference_id` minting and final `AdapterEvidenceReference` assembly — and Phase 3 — the authenticated gateway client — are all implemented and merged; Phase 4 onward has not begun. The gateway-side authentication boundary the producer runtime depends on is complete for its bounded scope (`basis-gateway` Phase 1B — 1B.1, 1B.2, 1B.3 — merged), and `basis-adapters`' own evidence-material construction (ADR-0007 Stage 1) is likewise complete, per the bullets above; the bounded producer slice as a whole remains incomplete.
...
basis-adapters evidence-material construction      Released (ADR-0007 Stage 1)
        ↓
basis-gateway producer mTLS auth/admission         Merged (Phase 1B: 1B.1, 1B.2, 1B.3 — not a
        (trusted-ingress mTLS, URI SAN, admission)  tagged release; bounded, approved scope)
        ↓
basis-producer Phase 2A evidence retention         Implemented — merged (repository
        (digest-addressed retention + retrieval)     `basis-foundation/basis-producer` exists)
        ↓
...
basis-producer Phase 3 gateway client              Implemented — merged
        ...
        ↓
basis-producer Phase 4 REST adapter composition    Next — not yet implemented
        (wire RestAdapter.normalize() into the Phase 2/3 pipeline
...
This roadmap is architecture only. It does not authorize implementation work in any repository, does not create `basis-demo` or any other repository, and does not select an identity-provider vendor as a runtime dependency of the BASIS Core Services Distribution. It depends most directly on **Next Producer and Execution-Evidence Boundary** above, which is now only partially unresolved: [ADR-0008](docs/adr/0008-producer-workload-authentication-and-admission.md) is `Accepted` and implemented for the bounded producer-authentication path — `basis-gateway`'s Phase 1B is merged, `basis-producer` now exists, and its Phase 2A, Phase 2B, and Phase 3 are all implemented and merged. Its own Phase 6 (the full identity-to-operation learning flow) cannot proceed until `basis-producer`'s remaining Phase 4 adapter composition — driving that authenticated submission from a real, adapter-normalized operation rather than a manually assembled one — is implemented, along with execution-result evidence and the other downstream prerequisites named under **Next Producer and Execution-Evidence Boundary** above, per [`docs/architecture/operation-producer-and-execution-boundary.md`](docs/architecture/operation-producer-and-execution-boundary.md). A narrower authentication-only reference lab (its Phase 5) is, by contrast, groundable in capability already released today — `basis-identity`'s `v0.1.0` OIDC, JWKS, session, and BASIS-local token surface — and the roadmap document is explicit about that distinction rather than presenting the two as equally available. It reconciles explicitly with the identity and fine-grained authorization expansion roadmap, the identity-to-operation contract and interoperability roadmap, the post-authentication identity activity roadmap, and the operator and training experience roadmap, consuming capability each of them owns rather than redefining any of it. See the roadmap document itself for phase-by-phase prerequisites, decision gates, and its `basis-demo` responsibility and trust-boundary model.
