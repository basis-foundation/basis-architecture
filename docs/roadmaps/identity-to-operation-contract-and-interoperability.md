...
**Status:** Planned. This roadmap's original gate — the `basis-gateway` operation-aware audit-evidence, readiness, conformance, and release-hardening program described in [`ROADMAP.md`](../../ROADMAP.md) — is satisfied: that program is complete and released (`v0.2.0`, feature-flagged). Satisfying that gate does not, by itself, start this roadmap's implementation. No phase described in this document is active, and this roadmap does not authorize implementation work in `basis-core`, `basis-gateway`, `basis-identity`, `basis-adapters`, `basis-console`, or `basis-schemas`. The ecosystem's active work is the next remaining integration boundary — trusted operation-producer identity and `basis-adapters` context/evidence alignment, per `ROADMAP.md`'s **Next Producer and Execution-Evidence Boundary** — which [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md) addresses as a first, bounded architecture application of this roadmap's invariants (see **Relationship to Existing Roadmaps** below); this document's own broader ten-phase program remains unstarted. Contract publication in `basis-schemas` is deferred until this roadmap's architecture and at least one reference implementation stabilize, following the same readiness discipline ADR-0005 established for the operation-aware contract suite. Repository names used in this document — including names such as a hypothetical relationship, evidence, or distribution service — are conceptual references to future architectural roles unless the repository already exists in the BASIS Core Services Distribution today (`basis-core`, `basis-gateway`, `basis-console`, `basis-adapters`, `basis-identity`, `basis-schemas`, `basis-architecture`; `basis-deploy` remains defined in architecture but not yet established as a repository, consistent with [`ROADMAP.md`](../../ROADMAP.md)).
...
[`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md) is the first document to narrow part of this roadmap — principally Phase 2's producer model and Phase 4's enforcement-and-execution lifecycle — into a single, more concrete architecture document, scoped to the specific gap `ROADMAP.md` names as the next remaining integration boundary. It does not complete this roadmap's ten phases and does not change this roadmap's own "Planned" status; it is a first, bounded application of this roadmap's invariants and contract responsibility model, and its own decision gates (which authentication mechanism, whether a new repository is required, what the execution-status vocabulary should be) remain open in the same sense this roadmap's broader phases do.
...
basis-gateway operation-aware integration, audit-evidence,
readiness, conformance, and release-hardening program
    (this roadmap's original gate — satisfied: complete and
    released, v0.2.0, feature-flagged. Satisfying it does not,
    by itself, start Phases 2 through 10 below; no phase of
    this roadmap is currently active. The ecosystem's active
    work is the next remaining integration boundary — see
    docs/architecture/operation-producer-and-execution-boundary.md)
    ↓
Phase 2: identity, authority, delegation, and producer model
    ↓
Phase 3: canonical operation and resource semantics
...
This sequence is provisional. It reflects the dependency structure named in each phase's Prerequisites subsection above, not a committed schedule or an exact pull-request count. Phase 1 is set apart from the chain below it because it is architecture discovery only — cataloguing what already exists — and carries no dependency on the gateway program; it is drawn this way specifically so the diagram does not read as placing discovery work behind that program's completion, which would contradict the Prerequisites stated in Phase 1 above. The gateway program itself — this roadmap's original gate — is satisfied, as the Status section at the top of this document states; satisfying it does not, by itself, start Phases 2 through 10, and nothing in this diagram authorizes any of them to begin now. Beginning them requires a separate architecture and implementation decision, consistent with how [`ROADMAP.md`](../../ROADMAP.md) treats every other gated program in the ecosystem. Architecture discoveries made during any phase may still justify splitting a phase, combining two phases, or reordering later phases — Phase 6's integration profiles, for instance, could reasonably begin in parallel with Phase 5 once Phase 3 and Phase 4 are stable, since integration profiles depend more on the operation and lifecycle model than on the transport mechanism. Any such change to phase sequencing that affects a stable component boundary or an established contract should go through the ADR process described in [`GOVERNANCE.md`](../../GOVERNANCE.md), the same discipline [`ROADMAP.md`](../../ROADMAP.md) already applies to changes in architectural intent.

To be explicit about what this sequencing does and does not authorize: no phase of this roadmap is currently active; the `basis-gateway` operation-aware integration program that formed this roadmap's original gate is complete and released and is not reordered, paused, or deprioritized by this diagram — it is simply no longer the thing this roadmap is waiting on; Phase 1 is architecture discovery only and produces no implementation; and Phases 2 through 10 remain unstarted pending the separate architecture and implementation decision named above. This roadmap does not authorize implementation of any phase. The ecosystem's present architecture-planning work under this roadmap's umbrella is [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md), which narrows part of Phase 2 and Phase 4 into a first, bounded document without itself constituting Phase 2 or Phase 4 in full — see **Relationship to Existing Roadmaps** above.
...
- [`docs/roadmaps/identity-provider-integration-and-learning-environment.md`](identity-provider-integration-and-learning-environment.md) — the `basis-demo`/Training-Mode identity-learning roadmap whose end-to-end learning flow visualizes this roadmap's Contract Responsibility Model and is gated most directly on the operation-producer-and-execution-boundary work this roadmap's Phase 2 and Phase 4 narrow
