...
**Status:** Planned only. No implementation is authorized by this document, and no repository — including `basis-demo` — is created by it. This roadmap does not commit to a `basis-demo` creation date, a release schedule, or a specific identity-provider vendor as a runtime dependency of the BASIS Core Services Distribution. The ecosystem's active implementation priority is what [`ROADMAP.md`](../../ROADMAP.md) now states it to be: the bounded operation-producer authorization reference slice is complete, and the next major implementation chapter — most plausibly the execution-evidence boundary — awaits a separate architecture decision, under **Next Producer and Execution-Evidence Boundary**. [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) (producer workload authentication via mTLS) is recorded `Accepted`, and `basis-gateway`'s Phase 1B (1B.1, 1B.2, and 1B.3) has since merged in full — gateway-side producer mTLS authentication and admission is implemented for its bounded, approved scope. The operation-producer runtime itself — the component [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md), Accepted, permanently names `basis-producer` — exists as a repository and has implemented all five phases of its bounded reference implementation: Phase 2A (evidence-retention foundation), Phase 2B (reference lifecycle), Phase 3 (authenticated gateway client, submitting to `basis-gateway`'s operation-aware evaluation endpoint), Phase 4 (REST adapter composition), and Phase 5 (cross-repository conformance, demonstration, and release-readiness review). Nothing in this document reorders, pauses, deprioritizes, or competes with that work, with the current `basis-gateway` operation-aware rollout, or with any other roadmap's stated active priority. This roadmap's implementation program, taken as a whole, remains downstream of that work — but not every phase depends on it equally. Architecture-discovery work (Phase 1) and provider-neutral lab architecture (Phase 2 through Phase 4) may be reasoned about, and an authentication-only Keycloak learning scenario (Phase 5) may be architected, independently of it: `basis-identity` `v0.1.0` already provides real, released OIDC, JWKS, session, and BASIS-local token capability that scenario can be grounded in. The full identity-to-operation learning flow (Phase 6 onward) is no longer gated on producer composition — that is implemented — but remains gated on execution-evidence work that has not yet begun. None of this authorizes `basis-demo` implementation to begin, and none of it moves `basis-demo` ahead of the downstream rollout sequence `ROADMAP.md` already establishes; it only clarifies which parts of this roadmap's own architecture are, and are not, technically blocked on work still in progress elsewhere. Conceptual component and repository names used in this document — principally `basis-demo` — are conceptual references to a future architectural role. `basis-demo` is not a new name invented by this roadmap: [`ROADMAP.md`](../../ROADMAP.md)'s own Downstream Rollout Sequence already names "`basis-demo` end-to-end validation" as a future stage, positioned after `basis-deploy` packaging and before real OT integration validation. This roadmap elaborates the architectural shape of that already-named future stage specifically for identity-provider integration and identity-systems learning; it does not move that stage earlier in the sequence, and it does not create a second, competing demo concept.
...
**`basis-gateway` invariant.** Unchanged from [`docs/architecture/basis-gateway.md`](../architecture/basis-gateway.md): the gateway authenticates, composes, invokes, enforces, and emits evidence. A lab exercises the real gateway boundary — including the real, already-implemented gateway-side producer mTLS authentication and admission ADR-0008/ADR-0009 define (`basis-gateway` Phase 1B) — never a bypass of it. The producer-side mTLS client that drives that boundary, and its composition from a real, adapter-normalized operation, are likewise implemented and merged (`basis-producer` Phase 3 and Phase 4); a lab that exercises the full producer-to-gateway submission can now do so end to end.
...
[`docs/roadmaps/identity-to-operation-contract-and-interoperability.md`](identity-to-operation-contract-and-interoperability.md) provides the authoritative point-in-time semantics connecting subject, authority, delegation, producer, operation, resource, authorization, enforcement, execution, and evidence — the Contract Responsibility Model this roadmap's end-to-end learning flow (Phase 6) visualizes and exercises rather than redefines. The nearer, bounded application of that roadmap's Phase 2 and Phase 4 — [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md) and [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — is this roadmap's single most load-bearing prerequisite: the gateway side of that real producer-to-gateway trust boundary is implemented and merged (`basis-gateway` Phase 1B), and `basis-producer`'s own evidence-retention foundation, reference-lifecycle assembly, authenticated gateway client, and adapter composition (Phase 2A, Phase 2B, Phase 3, Phase 4) are all implemented and merged — the bounded authorization boundary can now be exercised end to end. This roadmap's later phases still cannot exercise a real execution attempt or real execution evidence, though, until the execution-evidence architecture — a separate, not-yet-made decision — is defined and implemented. This roadmap's training traces, once they exist, follow the same identity-to-operation contract and evidence chain that roadmap defines; they do not introduce a competing representation.
...
**`basis-adapters` / trusted producers.** Expose or contribute the existing trustworthy evidence necessary to explain normalized operations, producer identity, and producer admission — capabilities the gateway side already implements — plus, once they exist per the operation-producer-and-execution-boundary work, execution attempts and execution results. This roadmap does not expand `basis-adapters`' non-network, non-authenticating contract to accommodate a lab.
...
Steps 1 through 16 sit substantially within `basis-identity`'s existing, released `v0.1.0` scope — OIDC discovery, JWKS retrieval, ID-token verification, authorization-request construction with PKCE, callback validation, session establishment, and BASIS-local token issuance and signing are implemented and tested for that release, per [`ROADMAP.md`](../../ROADMAP.md). What is not yet true, and what this roadmap does not claim is true, is that this chain is integrated against the operation-aware surface — `basis-identity`'s evidence alignment against operation-aware evaluation is explicitly named in [`ROADMAP.md`](../../ROADMAP.md) as "Planned, not yet started." Steps 17 through 23 are each at a different point of readiness, not a single uniform "unimplemented" state:

- **Steps 17–18 (producer workload authentication and admission).** Both sides are implemented and merged: the trusted NGINX ingress, certificate handoff, URI SAN identity derivation, exact admission matching, and the dual producer/authorization-subject authentication boundary (ADR-0008, `Accepted`; ADR-0009, `Accepted`; `basis-gateway` Phase 1B — 1B.1, 1B.2, 1B.3); and the producer-side mTLS client presentation that drives that boundary — `basis-producer`'s Phase 3, merged.
- **Step 19 (operation normalization through a producer runtime).** `basis-producer` exists and its Phase 2A evidence-retention foundation, Phase 2B reference-lifecycle assembly, Phase 3 authenticated gateway submission, and Phase 4 adapter orchestration are all implemented and merged, per [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md)'s Stages 4 and 5. `basis-producer`'s Phase 5 conformance, demonstration, and release-readiness review is likewise implemented and merged.
- **Steps 22–23 (execution outcome and evidence retention/correlation).** Remain unimplemented; that document's §12 still records this as an open schema and runtime gap.

This roadmap's Phase 5 and Phase 6 exist specifically to separate these halves of the trace rather than presenting the flow as a single, uniformly available capability.
...
| Operation normalization via a producer runtime (step 19) | [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md) §13 Stages 4–5; [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) | Permanent component (`basis-producer`) established, `Accepted`; repository exists, Phase 2A (evidence-retention foundation), Phase 2B (reference lifecycle), Phase 3 (authenticated gateway client and submission), Phase 4 (adapter composition), and Phase 5 (conformance/demo/release-readiness) all implemented and merged — the bounded reference slice is complete for its approved scope |
...
**Architectural outcome.** A documented current-state chain — the same chain the **Required Dependency / Lab Availability Matrix** above summarizes — distinguishing released and implemented capability (`basis-identity` `v0.1.0`'s OIDC/JWKS/session/token surface; `basis-core`/`basis-gateway`/`basis-console` `v0.2.0`'s operation-aware surface; `basis-adapters` `v0.2.0`'s evidence-material construction; and the now-complete bounded operation-producer authorization slice — ADR-0008's gateway-side producer mTLS authentication/admission, and `basis-producer`'s Phase 2A evidence retention, Phase 2B reference lifecycle, Phase 3 authenticated gateway client, Phase 4 adapter composition, and Phase 5 conformance/demonstration/release-readiness review, all implemented and merged) from capability that is merely planned elsewhere (the identity/FGA and post-authentication roadmaps) and from the still-unresolved execution-evidence boundary.
...
**Prerequisites.** Phase 5; [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — now `Accepted` and, on the gateway side, implemented and merged (`basis-gateway` Phase 1B: 1B.1, 1B.2, 1B.3) — with the producer side of that same bounded reference slice (`basis-producer`'s Phase 3 mTLS gateway client and Phase 4 adapter composition) now also implemented and merged, on top of its already-implemented Phase 2A/Phase 2B evidence-retention and reference-lifecycle foundation; execution-result evidence and `basis-identity`'s evidence alignment against the operation-aware surface, per [`ROADMAP.md`](../../ROADMAP.md)'s own "Planned, not yet started" status for that work, remain outstanding. This phase is explicitly and substantially gated — it is named here as architecture, not as a near-term buildable increment.
...
**Key capabilities.** Everything Phase 5 provides, extended through: real mTLS-based producer authentication and admission per ADR-0008's bounded reference slice, already implemented and merged at the gateway (`basis-gateway` Phase 1B); a real operation-producer runtime (`basis-producer`) actually submitting a real operation-aware request over that boundary, using its implemented and merged Phase 3 mTLS gateway client and Phase 4 adapter composition — driving that client from a normalized, protocol-sourced request rather than a manually constructed one, now implemented; real `basis-core`/`basis-gateway` evaluation and enforcement, exactly as already released in `v0.2.0`; and, only once it exists, real execution-evidence production. This phase must not simulate any of the steps that still lack real capability — a lab that fabricates execution or execution evidence to complete the trace before the real execution architecture exists violates the **`basis-demo` invariant** and the Training-Mode constitutional requirement stated above, and is explicitly rejected as an interim substitute.
...
**Explicitly deferred work.** Everything gated on execution-result evidence and the execution/evidence correlation model the boundary document's §6 and §12 still name as unresolved, and on `basis-identity` evidence alignment where required — which is to say, most of this phase's own content, honestly stated. The bounded producer-authorization slice itself (ADR-0008's gateway side and `basis-producer`'s Phase 2A through Phase 5) is no longer part of what this phase defers — it is implemented and merged.
...
**Prerequisites.** [`docs/roadmaps/identity-and-fine-grained-authorization-expansion.md`](identity-and-fine-grained-authorization-expansion.md) Phase 3 (token exchange and delegation) and Phase 4 (workload and non-human identity) reaching implementation; Phase 6's producer-authentication lab (mTLS) as the nearest-term workload-identity scenario this roadmap can already ground in the accepted ADR-0008 architecture — implemented at the gateway (`basis-gateway` Phase 1B) and now implemented at the producer as well (`basis-producer` Phase 3 mTLS gateway client), pending only Phase 6's own end-to-end composition.

**Architectural outcome.** A workload-identity lab exercises the real mTLS producer-authentication profile ADR-0008 defines, now implemented at both the gateway and `basis-producer`'s Phase 3 client; a token-exchange or delegation lab exercises the real capability the identity/FGA roadmap's Phase 3 defines once it, too, is implemented. This phase invents no interim substitute for either.
...
| Producer authentication/admission (steps 17–18) | Producer and trust status (backing capability implemented at both gateway and producer; lab surface not yet built) | mTLS handshake, URI SAN identity, admission matching |
...
Phase 5: baseline Keycloak OIDC reference learning flow (authentication scope)
    ...
    ↓
Phase 6: end-to-end identity-to-operation learning flow
    (bounded producer authorization slice complete — ADR-0008 is
    Accepted and implemented at the gateway and throughout
    basis-producer's Phase 2A-5, including Phase 4 REST adapter
    composition; gated instead on execution-result evidence and the
    execution boundary docs/architecture/operation-producer-and-
    execution-boundary.md still names as unresolved — not a
    near-term phase)
    ↓
Phase 9: workload, machine, and delegation learning scenarios
...
This roadmap defines the missing bridge between an external identity provider, a real identity flow, BASIS authorization, enforcement, execution, and evidence, and safe, repeatable Training Mode inspection of all of it — the specific, narrow gap none of the four existing roadmaps in this repository already covers, because each of them governs a different layer of the same larger system. It assigns `basis-demo` a bounded orchestration and reference-scenario role that consumes the real ecosystem rather than reimplementing any part of it, and it holds that boundary as the single most important invariant in the document. It separates what is buildable today — an authentication-only Keycloak reference lab, grounded in `basis-identity`'s already-released `v0.1.0` capability — from what remains gated on architecture that is not yet decided or built. The bounded producer-authorization slice itself is complete: `basis-gateway` Phase 1B is implemented and merged, and `basis-producer`'s Phase 2A (evidence retention), Phase 2B (reference lifecycle), Phase 3 (authenticated gateway client), Phase 4 (REST adapter composition), and Phase 5 (cross-repository conformance, demonstration, and release-readiness review) are all implemented and merged. What remains gated is the execution boundary itself: execution-result evidence (which does not exist anywhere in the ecosystem), full authorization-to-execution correlation, and `basis-identity` operation-aware evidence alignment where required — and this roadmap refuses to claim otherwise for pedagogical convenience. It teaches two flagship invariants — authentication is not authorization, and authorization is not execution — using the ecosystem's own already-established architecture, implemented for the bounded authorization slice ADR-0008 and ADR-0009 authorize, rather than inventing new doctrine to make the point. It reconciles explicitly with all four existing roadmaps, naming what each already owns and what this roadmap merely consumes. And it remains, throughout, exactly what its status line states: planned only, architecture-planning work, not an implementation authorization, not a commitment to build `basis-demo` on any particular timeline, and not permission to interrupt the ecosystem's actual current priority.
...
- [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — the accepted producer-authentication decision, implemented at the gateway (`basis-gateway` Phase 1B) and at the producer (`basis-producer`'s Phase 3 mTLS gateway client and Phase 4 REST adapter composition, both merged); this roadmap's Phase 6 and Phase 9 are gated instead on execution-result evidence and the other prerequisites named under `ROADMAP.md`'s **Next Producer and Execution-Evidence Boundary**, since the bounded producer-authorization slice itself is complete
