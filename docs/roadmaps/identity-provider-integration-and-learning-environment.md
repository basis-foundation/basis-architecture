...
[`docs/roadmaps/identity-to-operation-contract-and-interoperability.md`](identity-to-operation-contract-and-interoperability.md) provides the authoritative point-in-time semantics connecting subject, authority, delegation, producer, operation, resource, authorization, enforcement, execution, and evidence — the Contract Responsibility Model this roadmap's end-to-end learning flow (Phase 6) visualizes and exercises rather than redefines. The nearer, bounded application of that roadmap's Phase 2 and Phase 4 — [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md) and [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — is this roadmap's single most load-bearing prerequisite: the gateway side of that real producer-to-gateway trust boundary is now implemented and merged (`basis-gateway` Phase 1B), and `basis-producer`'s own evidence-retention foundation, reference-lifecycle assembly, and authenticated gateway client (Phase 2A, Phase 2B, Phase 3) are all implemented and merged, but this roadmap's later phases still cannot exercise the full boundary end to end, a real execution attempt, or real execution evidence until `basis-producer`'s remaining Phase 4 adapter composition and the execution-evidence architecture are implemented. This roadmap's training traces, once they exist, follow the same identity-to-operation contract and evidence chain that roadmap defines; they do not introduce a competing representation.
...
- **Step 19 (operation normalization through a producer runtime).** `basis-producer` now exists and its Phase 2A evidence-retention foundation, Phase 2B reference-lifecycle assembly, and Phase 3 authenticated gateway submission are all implemented and merged. Adapter orchestration (Phase 4) remains unimplemented, per [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md)'s Stages 4 and 5.
...
| Operation normalization via a producer runtime (step 19) | [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md) §13 Stages 4–5; [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) | Permanent component (`basis-producer`) established, `Accepted`; repository exists, Phase 2A (evidence-retention foundation), Phase 2B (reference lifecycle), and Phase 3 (authenticated gateway client and submission) all implemented and merged; adapter composition (Phase 4) and later phases not yet implemented |
...
**Architectural outcome.** A documented current-state chain — the same chain the **Required Dependency / Lab Availability Matrix** above summarizes — distinguishing released capability (`basis-identity` `v0.1.0`'s OIDC/JWKS/session/token surface; `basis-core`/`basis-gateway`/`basis-console` `v0.2.0`'s operation-aware surface; `basis-adapters` `v0.2.0`'s evidence-material construction) from architecture that is `Accepted` and partially implemented (ADR-0008's gateway-side producer mTLS authentication/admission is implemented and merged; `basis-producer`'s Phase 2A evidence retention, Phase 2B reference lifecycle, and Phase 3 authenticated gateway client are all implemented and merged; adapter composition, Phase 4, remains unimplemented) from capability that is merely planned elsewhere (the identity/FGA and post-authentication roadmaps).
...
**Explicitly deferred work.** Everything gated on `basis-producer`'s remaining bounded reference-implementation work (Phase 4 REST adapter composition — both ADR-0008's gateway side and `basis-producer`'s own Phase 3 mTLS gateway client are already implemented and merged) and on `basis-identity` evidence alignment — which is to say, most of this phase's own content, honestly stated.
...
Phase 5: baseline Keycloak OIDC reference learning flow (authentication scope)
    ...
    ↓
Phase 6: end-to-end identity-to-operation learning flow
    (gated on basis-producer's remaining Phase 4 REST adapter
    composition — ADR-0008 is Accepted and already implemented at
    both the gateway and basis-producer's Phase 3 mTLS gateway client —
    and on docs/architecture/operation-producer-and-execution-boundary.md
    Stage 5's bounded reference implementation completing — not a
    near-term phase)
    ↓
Phase 9: workload, machine, and delegation learning scenarios
...
- [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — the accepted producer-authentication decision, implemented at the gateway (`basis-gateway` Phase 1B) and at the producer (`basis-producer`'s Phase 3 mTLS gateway client); this roadmap's Phase 6 and Phase 9 are gated on the remaining producer side of that same bounded slice, `basis-producer`'s still-unimplemented Phase 4 REST adapter composition
