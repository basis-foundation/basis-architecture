...
[`docs/roadmaps/identity-to-operation-contract-and-interoperability.md`](identity-to-operation-contract-and-interoperability.md) provides the authoritative point-in-time semantics connecting subject, authority, delegation, producer, operation, resource, authorization, enforcement, execution, and evidence — the Contract Responsibility Model this roadmap's end-to-end learning flow (Phase 6) visualizes and exercises rather than redefines. The nearer, bounded application of that roadmap's Phase 2 and Phase 4 — [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md) and [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — is this roadmap's single most load-bearing prerequisite: the gateway side of that real producer-to-gateway trust boundary is now implemented and merged (`basis-gateway` Phase 1B), and `basis-producer`'s own evidence-retention and reference-lifecycle foundation (Phase 2A, Phase 2B) is implemented, but this roadmap's later phases still cannot exercise the full boundary end to end, a real execution attempt, or real execution evidence until `basis-producer`'s remaining Phase 3 mTLS gateway client and the execution-evidence architecture are implemented. This roadmap's training traces, once they exist, follow the same identity-to-operation contract and evidence chain that roadmap defines; they do not introduce a competing representation.
...
- **Steps 17–18 (producer workload authentication and admission).** The gateway side is implemented and merged: the trusted NGINX ingress, certificate handoff, URI SAN identity derivation, exact admission matching, and the dual producer/authorization-subject authentication boundary (ADR-0008, `Accepted`; ADR-0009, `Accepted`; `basis-gateway` Phase 1B — 1B.1, 1B.2, 1B.3). What remains unimplemented is the producer-side mTLS client presentation that would drive that boundary — `basis-producer`'s Phase 3.
- **Step 19 (operation normalization through a producer runtime).** `basis-producer` now exists and both its Phase 2A evidence-retention foundation and its Phase 2B reference-lifecycle assembly are implemented and merged. Adapter orchestration and gateway submission (Phase 3) remain unimplemented, per [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md)'s Stages 4 and 5.
...
| Operation normalization via a producer runtime (step 19) | [`docs/architecture/operation-producer-and-execution-boundary.md`](../architecture/operation-producer-and-execution-boundary.md) §13 Stages 4–5; [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) | Permanent component (`basis-producer`) established, `Accepted`; repository exists, Phase 2A (evidence-retention foundation) and Phase 2B (reference lifecycle) both implemented; gateway submission (Phase 3) and later phases not yet implemented |
...
**Explicitly deferred work.** Everything gated on `basis-producer`'s remaining bounded reference-implementation work (Phase 3 mTLS gateway client — the gateway side of ADR-0008 is already implemented and merged) and on `basis-identity` evidence alignment — which is to say, most of this phase's own content, honestly stated.
...
**Prerequisites.** [`docs/roadmaps/identity-and-fine-grained-authorization-expansion.md`](identity-and-fine-grained-authorization-expansion.md) Phase 3 (token exchange and delegation) and Phase 4 (workload and non-human identity) reaching implementation; Phase 6's producer-authentication lab (mTLS) as the nearest-term workload-identity scenario this roadmap can already ground in the accepted ADR-0008 architecture — implemented at the gateway (`basis-gateway` Phase 1B), pending only `basis-producer`'s own Phase 3 mTLS gateway client to be exercised end to end.

**Architectural outcome.** A workload-identity lab exercises the real mTLS producer-authentication profile ADR-0008 defines, already implemented at the gateway, once `basis-producer`'s Phase 3 client makes it exercisable end to end; a token-exchange or delegation lab exercises the real capability the identity/FGA roadmap's Phase 3 defines once it, too, is implemented. This phase invents no interim substitute for either.
...
Phase 5: baseline Keycloak OIDC reference learning flow (authentication scope)
    ...
    ↓
Phase 6: end-to-end identity-to-operation learning flow
    (gated on basis-producer's remaining Phase 3 mTLS gateway client —
    ADR-0008 is Accepted and already implemented at the gateway —
...
- [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — the accepted producer-authentication decision, implemented at the gateway (`basis-gateway` Phase 1B); this roadmap's Phase 6 and Phase 9 are gated on the producer side of that same bounded slice, `basis-producer`'s still-unimplemented Phase 3 mTLS gateway client
