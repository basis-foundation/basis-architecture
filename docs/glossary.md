...
The conceptual expansion of `basis-core` from a basic subject/action/resource authorization kernel into a kernel that evaluates richer OT operation context — subject attributes, resource type, site/zone, device identity and class, protocol operation evidence, operation intent, safety/risk/environment/time context, and correlation metadata — while remaining protocol-agnostic, identity-provider-agnostic, and enforcement-agnostic. The model is defined in [`docs/architecture/operation-aware-authorization-model.md`](architecture/operation-aware-authorization-model.md) and adopted in ADR-0001. It defines conceptual categories for a richer `DecisionRequest`/`DecisionResponse`, not a policy language. `basis-schemas` v0.2.0 has since published the machine-readable `operation-aware-decision-request` and `operation-aware-decision-response` contracts that formalize those categories; `basis-core` v0.2.0 implements evaluation against them. See also: **Authorization Kernel**, **Policy Evaluation**, **basis-core**.

---

## Operation-Aware Evaluation Semantics

The set of deterministic rules governing how `basis-core` reasons about the richer context described by the **Operation-Aware Authorization Model** — default deny, `NOT_APPLICABLE` behavior, deny precedence, conflict resolution, rule ordering, condition evaluation, missing context behavior, unknown action/resource behavior, schema version compatibility, deterministic trace output, and safe error handling. These semantics are defined in [`docs/architecture/operation-aware-evaluation-semantics.md`](architecture/operation-aware-evaluation-semantics.md) and adopted in ADR-0002. They define conceptual evaluation behavior, not a policy language. `basis-schemas` v0.2.0 has since published the `evaluation-trace` and `operation-aware-decision-response` contracts reflecting this behavior; `basis-core` v0.2.0 implements these semantics at runtime. See also: **Operation-Aware Authorization Model**, **Policy Evaluation**, **Authorization Decision**.

---

## Operation-Aware Policy Bundle and Rule Model

The conceptual model defining how policy is packaged, identified, validated, and evaluated for the **Operation-Aware Authorization Model** — the **Policy Bundle** as the unit of policy distribution, versioning, validation, and compatibility; policy scope; the rule as a deterministic unit of evaluation with an effect, match criteria, conditions, a reason code, and traceable metadata; `ALLOW`/`DENY` rule effects and their relationship to deny precedence; bundle and rule combining semantics; rule ordering; policy validation; and policy metadata and provenance. The model is defined in [`docs/architecture/operation-aware-policy-rule-model.md`](architecture/operation-aware-policy-rule-model.md) and adopted in ADR-0004. It defines conceptual policy model categories and ownership, not a policy language. `basis-schemas` v0.2.0 has since published the `policy-condition`, `policy-rule`, and `policy-bundle` contracts that formalize the structural shape of this model; the condition-operator semantics themselves — what a condition's `operator` actually evaluates at runtime — are addressed by **Condition Operator Semantics**, an approved clarification implemented by `basis-core` v0.2.0. See also: **Operation-Aware Authorization Model**, **Operation-Aware Evaluation Semantics**, **Condition Operator Semantics**, **Policy Bundle**, **Policy Engine**, **Policy Evaluation**.

---

## Condition Operator Semantics

The approved, `basis-core`-v0.2.0-scoped set of ten operators (`equals`, `not_equals`, `in`, `not_in`, `greater_than`, `greater_than_or_equal`, `less_than`, `less_than_or_equal`, `exists`, `not_exists`) and their complete evaluation semantics — field-path resolution against `OperationAwareDecisionRequest`, absent/null handling, type-mismatch handling, and condition-array evaluation order — that the `basis-core` v0.2.0 condition evaluator implements without inventing semantics of its own. Defined in [`docs/architecture/condition-operator-semantics.md`](architecture/condition-operator-semantics.md), a narrow architecture clarification (not an ADR) that narrows, but does not amend or contradict, ADR-0004 §7's deliberate deferral of a full operator language. It does not close `policy-condition`'s `operator` field as a permanent ecosystem-wide enum; it defines the first implementable subset only. Approved and implemented; `basis-core` condition-evaluation code (Milestone 7, PRs 22-23 of the v0.2.0 roadmap) implements this ten-operator registry. See also: **Operation-Aware Policy Bundle and Rule Model**, **Policy Evaluation**.

---

## Operation-Aware Schema Readiness

The architecture handoff plan defining how the **Operation-Aware Authorization Model**, **Operation-Aware Evaluation Semantics**, **Operation-Aware Trace and Audit Evidence**, and **Operation-Aware Policy Bundle and Rule Model** should move into `basis-schemas` as machine-readable contracts — which contract surfaces should be published, in what order, under what compatibility rules, with what examples and test vectors, and what must remain deferred. Also referred to as the Schema Readiness Plan. The plan is defined in [`docs/architecture/operation-aware-schema-readiness-plan.md`](architecture/operation-aware-schema-readiness-plan.md) and adopted in ADR-0005. It defines a publication sequence and compatibility expectations, not a policy language or implementation code. That sequence has since been executed: `basis-schemas` v0.2.0 publishes all fourteen operation-aware contracts named by the plan, plus five canonical compatibility scenarios connecting them. See also: **Operation-Aware Authorization Model**, **Operation-Aware Evaluation Semantics**, **Operation-Aware Policy Bundle and Rule Model**, **Operation-Aware Trace and Audit Evidence**.

---

## Operation-Aware Trace and Audit Evidence

The conceptual model defining what evidence must exist to make an operation-aware decision explainable, auditable, testable, and safe to visualize — including the distinction between kernel-produced **evaluation trace**, durable **audit evidence**, and the runtime **gateway audit event**; the evidence lifecycle from identity context through console visualization; conceptual categories for trace, rule-level evidence, request evidence, adapter evidence references, and identity evidence references; redaction tiers; and reason codes. The model is defined in [`docs/architecture/operation-aware-trace-audit-evidence.md`](architecture/operation-aware-trace-audit-evidence.md) and adopted in ADR-0003. It defines conceptual evidence categories and ownership, not a policy language. `basis-schemas` v0.2.0 has since published the `evaluation-trace`, `trace-rule-evidence`, `audit-evidence`, `gateway-audit-event`, and `reason-code` contracts that formalize this trace/audit separation as machine-readable shapes; runtime production of that evidence by `basis-core` is implemented (v0.2.0), while production by `basis-gateway`, `basis-adapters`, and `basis-identity` remains in progress. A published contract does not by itself prove the evidence it describes is trustworthy or authentic — that depends on the producing component. See also: **Operation-Aware Authorization Model**, **Operation-Aware Evaluation Semantics**, **Immutable Audit Logging**.
...
The conceptual unit of policy distribution, versioning, validation, and compatibility in the **Operation-Aware Policy Bundle and Rule Model** — the thing `basis-core` evaluates a `DecisionRequest` against and the thing `basis-gateway` loads and selects at runtime. A policy bundle carries a bundle identifier, bundle version, schema version, policy owner/authority, target scope, rule set, metadata, compatibility metadata, validation status, provenance, and optional deprecation/replacement metadata. A policy bundle is not necessarily a file format. `basis-schemas` v0.2.0 has since published a concrete serialization as the `policy-bundle` contract, alongside the dependent `policy-rule` and `policy-condition` contracts. Defined in [`docs/architecture/operation-aware-policy-rule-model.md`](architecture/operation-aware-policy-rule-model.md) and adopted in ADR-0004. See also: **Operation-Aware Policy Bundle and Rule Model**, **Policy Engine**, **Policy Evaluation**.
