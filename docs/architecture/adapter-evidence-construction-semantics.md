...
the final `AdapterEvidenceReference`. The `basis-producer` repository now exists, and all five
implementation phases of its bounded reference implementation are implemented and merged: Phase 2A
(evidence-retention foundation) — it retains adapter-constructed evidence bytes, digest-addressed,
and verifies them on retrieval — Phase 2B (reference-lifecycle work: `reference_id` minting,
reference binding, and final `AdapterEvidenceReference` assembly), Phase 3 (authenticated gateway
client: independent mTLS producer and bearer authorization-subject credentials, submission against
`basis-gateway`'s existing operation-aware endpoint), Phase 4 (REST adapter composition: wiring
`RestAdapter.normalize()` into that pipeline end to end), and Phase 5 (cross-repository conformance,
demonstration, and release-readiness review). `basis-producer` now assembles `AdapterEvidenceReference`
instances and submits them to `basis-gateway`, orchestrating `basis-adapters` normalization to
produce that evidence from a live protocol operation — the bounded authorization slice is complete;
...
| 6 — Reference implementation validation — **COMPLETE (Phase 4 and Phase 5, in `basis-producer`'s own numbering)** | Whichever repositories Stages 1–5 touched | Stages 1–5 | End-to-end validation that a real evidence artifact, constructed per this document, flows from adapter normalization through producer assembly to gateway admission without reinterpretation. Stages 1–5 were each independently complete; what remained was composing them into one live path: `ProtocolOperation` → `RestAdapter.normalize()` → evidence construction → producer retention/reference lifecycle → authenticated gateway submission → gateway/core disposition → STOP — this was `basis-producer`'s Phase 4, now complete. Broader cross-repository conformance, beyond this bounded composition, was `basis-producer`'s Phase 5, likewise complete | Does not add execution or execution-evidence (out of scope of this entire document, per `operation-producer-and-execution-boundary.md`) | **Done.** Phase 4 composed the independently-implemented stages above into one live, adapter-normalized path (`RestOperationComposer.submit()`); Phase 5 produced a bounded, reproducible demonstration analogous to `basis-gateway`'s existing `demo/operation-aware/` (`demo/bounded-slice/run_demo.py`), plus cross-repository conformance tests and a release-readiness review |
