...
the final `AdapterEvidenceReference`. The `basis-producer` repository now exists, and its first two
implementation phases are both implemented and merged: Phase 2A (evidence-retention foundation) —
it retains adapter-constructed evidence bytes, digest-addressed, and verifies them on retrieval —
and Phase 2B (reference-lifecycle work: `reference_id` minting, reference binding, and final
`AdapterEvidenceReference` assembly). `basis-producer` now assembles `AdapterEvidenceReference`
instances; it does not yet submit them to `basis-gateway` (Phase 3).
...
lets a future verifier proceed without guessing which rules produced a given digest's input.
Verification, when a future evidence-verification component exists, proceeds as:

```text
1. retrieve the evidence artifact (the retained evidence material, from whatever
   future evidence store holds it — §7, §16);
...
schema field exists. Whether a future evidence-storage architecture actually guarantees (a) is
explicitly not decided by this document (§16); this document requires only that, whenever an
...
| Persist full evidence                 |                — |                 — |       — |         **Owns** (future) |
| Verify stored evidence against digest |                — |                 — |       — |         **Owns** (future) |
...
authentication (`basis-gateway`'s existing `OPERATION_PRODUCER_SUBJECT_IDS` classification, and
whatever supersedes it per `operation-producer-and-execution-boundary.md` §13 Stage 4), transport
integrity, and later evidence verification remain separate controls this document does not
substitute for or weaken.
...
material as constructed (§4) — the same representation that a future evidence store, if one exists,
would retain for later inspection under whatever `redaction_classification` the reference carries.
...
| Evidence persistence failure (a future evidence store cannot durably retain material it was asked to) | A distinct category from all of the above — an evidence-integrity/operational concern for whatever future component owns persistence (§7), not a normalization or construction failure. This document does not decide its handling beyond noting it is governed by the same `required`/`optional` deployment evidence mode, not treated as an automatic required-evidence-construction failure irrespective of mode. |
...
| Normalized-output substitution | Adapter → producer runtime handoff | The digest covers normalized output as part of the evidence material (§4); a substitution after digesting would produce a digest mismatch upon any future verification | No verification occurs today, because no future evidence-verification component exists yet (§16); until one does, substitution after construction is not detectable in practice, only detectable in principle | Operation-producer runtime (must not alter material after receiving it); future evidence-verification component |
...
| Canonicalization ambiguity | Any two independent implementations of the pure helper | §5 fixes the canonicalization profile exactly (`rfc8785`, RFC 8785 JCS) rather than leaving it open: two conforming implementations cannot disagree on canonical bytes for the same material, because both are conforming to the same external specification, not to a `basis-architecture`-invented scheme | The profile decision itself is closed, but no reference implementation of the pure helper exists yet (§17); until one does and is tested against RFC 8785's own test vectors, "conforming" is a requirement this document states, not yet a property anything has demonstrated in code | `basis-architecture` (for the requirements), a future reference implementation (for conformance) |
| Digest algorithm downgrade | Verifier ↔ evidence relationship | Algorithm identity travels with every digest value (§6); a verifier is never forced to trust an unlabeled or ambiguous algorithm | A verifier that accepts a weak algorithm label without deployment-level policy could still be induced to trust weak evidence — algorithm *acceptance* policy is a deployment/verifier concern this document does not govern | Future evidence-verification component; deployment policy |
| Digest substitution | Producer runtime → gateway → any future evidence store | The reference's `evidence_digest` is exactly what a future verifier recomputes against; a substituted digest would be detectable the moment verification against real evidence material occurs | No verification path exists yet, so substitution is currently undetectable in practice — the same residual risk as normalized-output substitution above | Operation-producer runtime; `basis-gateway` (admission only, not regeneration — §7); future evidence-verification component |
...
| Evidence deletion after a reference was emitted | Future evidence storage | Not mitigated by this document — no persistence mechanism is defined yet (§16) | A `reference_id` pointing to evidence that was never retained, or was later deleted, cannot be verified after the fact; this is an open question for whatever future evidence-storage architecture emerges | Future evidence-storage component |
...
| - | - | - | - | - | - | - |
| Reference-level profile field (as distinct from the evidence-material-internal fields resolved above) | `basis-adapters` (once a reference implementation exists) | A future evidence-verification component; a future conformance test kit | `basis-architecture` (this document; a future ADR would formalize) | A consumer needs to filter or route references by declared profile without retrieving and canonicalizing the full evidence material first | Additive if introduced; existing `adapter-evidence-reference` fields are unaffected | The evidence-material-internal fields (§9) already resolve the *verification* need; this row records only the narrower, distinct *reference-level convenience* question, which this document does not find a concrete consumer for today. |
| Stored-evidence locator semantics | A future evidence-storage or evidence-service component | `basis-console` (for operator display), a future audit/investigation surface | Not yet decided — §7 assigns future persistence to "future evidence storage," not to a named component | Resolving `reference_id` back to full, retained evidence material for operator review or forensic investigation | Additive if introduced, and entirely gated on a persistence architecture that does not exist yet | No existing contract defines evidence storage or retrieval at all — `adapter-evidence-reference.md` explicitly disclaims this ("does not define... evidence storage, evidence retrieval") |
...
- **Stored-evidence locator semantics**: no schema change is justified at this time — no
  persistence architecture exists for a locator to describe.
...
| - | - | - | - | - | - |
| 1 — `basis-adapters` evidence-material profile and pure helper | `basis-adapters` | This document's adoption (via its companion ADR) | Implement the evidence-material construction, canonicalization, and digest-computation helper this document assigns to `basis-adapters` (§4, §5, §7), only because this ADR assigns material construction and digest calculation to `basis-adapters` | Does not add producer authentication, a gateway client, `reference_id` minting, `adapter_source` selection, or `redaction_classification` assignment — all remain producer-runtime responsibilities per §7 | The helper is deterministic (same input → same output across repeated calls and process restarts) and produces `evidence_digest`-shaped output matching the published `adapter-evidence-reference.evidence_digest_shape` |
| 2 — Conformance tests across all nine adapters | `basis-adapters` | Stage 1 | Verify that the same semantic input produces stable canonical bytes and digest for every currently published protocol (REST, BACnet, Modbus, OPC UA, MQTT, DNP3, IEC 61850, KNX, Niagara) | Does not test producer-runtime or gateway behavior — scoped to the pure helper only | Cross-protocol contract tests extending `tests/test_normalization_contract.py`'s existing pattern pass for all nine adapters |
| 3 — Operation-producer runtime architecture | `basis-architecture` (a future, separate architecture document) | Stage 1 exists in principle (the ownership split this document defines does not require Stage 1 to be complete before the producer-runtime architecture is drafted, but a drafted architecture should assume Stage 1's shape) | Repository placement (per `operation-producer-and-execution-boundary.md` §11's still-open decision gate), lifecycle, and authentication decision for the operation-producer runtime role | Does not select a producer-authentication mechanism (already an explicit non-goal of `operation-producer-and-execution-boundary.md`, restated here) | A reviewed architecture document naming where the runtime lives and how it authenticates |
| 4 — Producer-side `AdapterEvidenceReference` assembly | Wherever Stage 3 places the operation-producer runtime | Stages 1 and 3 | `reference_id` minting, `adapter_source`, `normalization_version`/`mapping_version` carry-forward, `redaction_classification`, and request/correlation linkage (§7's producer-runtime column) | Does not implement digest computation (already done in Stage 1) or gateway admission logic | A reference implementation assembling valid `AdapterEvidenceReference` objects against the published schema for at least one protocol |
| 5 — `basis-gateway` admission and audit refinement | `basis-gateway` | Stage 4 | Only where current behavior is insufficient — `basis-gateway` already accepts and admits `adapter_evidence_reference` from a classified trusted producer (§operation-aware-endpoint.md); this stage addresses any gap Stage 4's reference implementation actually surfaces, not a speculative one | Does not regenerate or reinterpret adapter facts (§7); does not select producer authentication (Stage 3's concern) | Existing or extended gateway tests demonstrate correct admission and audit recording of a real, Stage-4-produced reference |
| 6 — Reference implementation validation | Whichever repositories Stages 1–5 touched | Stages 1–5 | End-to-end validation that a real evidence artifact, constructed per this document, flows from adapter normalization through producer assembly to gateway admission without reinterpretation | Does not add execution or execution-evidence (out of scope of this entire document, per `operation-producer-and-execution-boundary.md`) | A bounded, reproducible demonstration analogous to `basis-gateway`'s existing `demo/operation-aware/` |
| 7 — `basis-schemas` proposal | `basis-schemas` | Stage 6 | Only if implementation proves a real contract gap — specifically, only if Stage 6 demonstrates that the evidence-material profile identifier or canonicalization-profile-version gap named in §16 is real and needs a governed field | Does not publish a schema speculatively ahead of a demonstrated need | A `basis-schemas` PR, reviewed under the existing ADR-0005-style readiness discipline, citing a specific implementation gap Stage 6 surfaced |
