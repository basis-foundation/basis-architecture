...
- **Reference-lifecycle behavior (`reference_id` minting, reference binding, `adapter_source`/`redaction_classification` assignment, `AdapterEvidenceReference` assembly) is now implemented** — Phase 2B, merged.
- **Producer-side gateway-client behavior is now implemented** — `basis-producer` → mTLS client presentation → separate authorization-subject credential → gateway submission (Phase 3, merged).
...
The end-to-end path this diagram describes is still incomplete — not because the gateway side or the producer's gateway client need further work, but because nothing in the ecosystem yet drives this path from a real, adapter-normalized operation: `basis-producer` has implemented evidence retention, reference lifecycle, and an authenticated gateway client (Phase 2A, Phase 2B, Phase 3), each independently exercised and tested, but has not yet orchestrated `RestAdapter.normalize()` output through that pipeline end to end (Phase 4).
...
A deterministic, offline demo/test environment, structured analogously to `basis-gateway`'s existing `demo/operation-aware/` (README, a run script, checked-in policy/expected fixtures) but living inside `basis-producer` and driving both `basis-adapters` and a locally-configured `basis-gateway` instance — Phase 5 work, not yet built:
...
| `basis-adapters` (as a library dependency) | `basis-producer` | This is the entire point of the slice — reuse, not reimplementation, of normalization and evidence construction | Runtime | Already published/installable; no version-pinning concern beyond the repository's existing compatibility discipline; not yet added — `basis-producer`'s merged Phase 2A/2B/3 implementation does not import `basis_adapters` (confirmed: `pyproject.toml` runtime `dependencies` list `basis-core`, `pydantic`, and `httpx` only), pending Phase 4 adapter orchestration |
...
### Phase 4 — REST vertical slice — NEXT
...
5. Release-readiness assessment per repository (§27) — likely: no release required for `basis-adapters`, `basis-core`, or `basis-schemas`; a `basis-gateway` release is likely once Phase 1B lands; `basis-producer` itself is not released to any package index.
6. The permanent-repository maturity review (§30) — repository *existence* is settled by ADR-0010; §30 now evaluates readiness for broader responsibility.
...
- ~~Permanent operation-producer-runtime repository placement~~ (§30) — **resolved:** [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) establishes `basis-producer` as the answer and is recorded `Accepted`. The repository now exists and Phase 2A, Phase 2B, and Phase 3 are all implemented and merged; §30's maturity checklist remains the open item, and Phase 4 (REST vertical slice) is the next unresolved implementation phase.
...
| Local demo / conformance evidence | No | `basis-producer` | Phase 5 |
