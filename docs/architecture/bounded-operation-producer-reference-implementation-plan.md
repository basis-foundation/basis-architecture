...
**Status: Implementation complete for the bounded reference slice.** ADR-0008 is Accepted. Phase 1 of the bounded producer reference slice — the `basis-gateway`-side mTLS trust boundary, §26 Phase 1A and Phase 1B (1B.1, 1B.2, 1B.3) — has merged and is complete for its bounded, approved scope. [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) — Accepted — establishes `basis-producer` as the permanent repository and component for the operation-producer runtime this plan targets; the `basis-foundation/basis-producer` repository now exists, and all five phases of this plan's implementation PR sequence are implemented and merged:

```text
Phase 1B — gateway producer mTLS boundary            COMPLETE
Phase 2A — evidence retention                        COMPLETE
Phase 2B — reference lifecycle                        COMPLETE
Phase 3  — authenticated gateway client               COMPLETE
Phase 4  — REST adapter composition                   COMPLETE
Phase 5  — conformance/demo/release-readiness         COMPLETE
```

**Phase 4 — REST vertical slice** (`RestAdapter.normalize()` wired end to end through evidence construction, retention, reference assembly, dual authentication, admission, gateway, and kernel, stopping before any protocol execution) is implemented and merged (`basis-foundation/basis-producer` PR #4). **Phase 5 — cross-repository conformance, demonstration, and release-readiness review** is likewise implemented and merged (PR #5): an eight-category conformance workflow (`adapter-normalization`, `evidence-integrity`, `reference-lifecycle`, `producer-mtls-admission`, `bearer-subject-independence`, `deterministic-allow`, `deterministic-deny`, `no-execution`), a reproducible `allow`/`deny`/`invalid-bearer`/`unadmitted-producer` demonstration with evidence inspection and independent digest verification, and a release-readiness review concluding the repository is ready for an initial, explicitly bounded, pre-v1 release of its authorization-only scope — not a complete production operation-producer runtime. **The bounded operation-producer authorization reference slice this plan defines is therefore complete.** This document has served its purpose as the implementation source of truth for that slice; it is retained as a historical implementation record, not reopened to define a "Phase 6" — the reference slice deliberately stops at the authorization disposition, and no protocol execution is implemented, authorized, or planned by this document. The next architectural chapter — the authorization-to-execution and execution-evidence boundary — is a separate, not-yet-made architecture decision; see [`ROADMAP.md`](../../ROADMAP.md)'s **Next Producer and Execution-Evidence Boundary** section.
...
- targets the permanently established `basis-producer` repository for the operation-producer runtime, per [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) (Accepted) — the repository now exists, its Phase 2A (evidence-retention foundation), Phase 2B (reference lifecycle), and Phase 3 (producer gateway client) are all implemented and merged, and its placement is no longer provisional;
...
REST operation (local mapping fixture, no network target)
    ...
    → mint opaque reference_id (reference producer; implemented, Phase 2B)
    → durably persist reference_id → evidence-digest/storage-key binding (reference producer; implemented, Phase 2B)
    → confirm reference binding succeeded
    → assign adapter_source, redaction_classification (reference producer; implemented, Phase 2B)
    → assemble AdapterEvidenceReference (reference producer; implemented, Phase 2B)
    → establish mTLS producer connection to basis-gateway (reference producer: implemented, Phase 3; basis-gateway: implemented and merged, Phase 1B)
    ...
    → gateway performs exact admission match (basis-gateway; implemented and merged, Phase 1B.1)
...
- **Reference-lifecycle behavior (`reference_id` minting, reference binding, `adapter_source`/`redaction_classification` assignment, `AdapterEvidenceReference` assembly) is implemented** — Phase 2B, merged.
- **Producer-side gateway-client behavior is implemented** — `basis-producer` → mTLS client presentation → separate authorization-subject credential → gateway submission (Phase 3, merged).
- **Gateway-side behavior is implemented and merged** (Phase 1B): trusted NGINX ingress → certificate handoff → URI SAN derivation → exact admission → independent bearer-subject authentication (§13) → operation-aware composition → `basis-core` invocation → enforcement/audit, without alteration to kernel-invocation or audit-emission code.
- **Adapter composition driving the above from a real, adapter-normalized operation is implemented** — `RestAdapter.normalize()` output orchestrated end to end through evidence construction, Phase 2A/2B, and Phase 3 submission (Phase 4, merged), with no execution code added anywhere.
- **Cross-repository conformance, demonstration, and release-readiness review is implemented** — an eight-category conformance workflow, a reproducible `allow`/`deny`/`invalid-bearer`/`unadmitted-producer` demonstration, and a release-readiness review finding the repository ready for an initial, bounded, pre-v1 release of this authorization-only scope (Phase 5, merged).

**The end-to-end bounded authorization path described by this diagram is now implemented and proven for the REST reference slice.** `basis-producer` retains evidence, assembles the reference, authenticates to the gateway, and drives all of it from a real `RestAdapter.normalize()` output (Phase 2A, Phase 2B, Phase 3, Phase 4), against the implemented gateway trust boundary (Phase 1B), with the whole path conformance-tested and demonstrated (Phase 5). Nothing downstream of the authoritative disposition is implemented: this diagram still stops at `STOP — no protocol execution`, and that stop is unchanged by Phase 4 or Phase 5 landing.
...
## 5. Component Placement Decision and Historical Alternatives

This section originally evaluated the slice's most consequential open decision — where the operation-producer runtime should live. That decision is no longer open: [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) — Accepted — permanently establishes `basis-producer` as the answer. The five candidate locations below, and the reasoning that ruled out four of them, are preserved as architectural history — the analysis that justified the decision ADR-0010 now makes permanent — not as an open evaluation to be revisited by an implementing team. Evaluated against: architectural clarity; dependency direction; risk of role conflation; ease of later extraction; packaging implications; testability; risk that production code begins depending on reference code; whether the location implies permanent ownership; whether it requires circular dependencies; release/versioning consequences.
...
**Disposition:** Rejected. This option is preserved as historical record only. It is not an implementation-team fallback: ADR-0010 (Accepted) makes `basis-producer` the permanent component/repository decision, and revisiting this option would require a new architecture decision, not a cost-driven implementation choice during Phase 2.
...
| `reference_id` minting (post-retention) | `basis-producer` (implemented, Phase 2B) |
| `reference_id` → digest/storage-key binding | `basis-producer` (implemented, Phase 2B) |
| `adapter_source` assignment | `basis-producer` (implemented, Phase 2B) |
| `redaction_classification` assignment | `basis-producer` (implemented, Phase 2B, under fixed reference-deployment policy — §10) |
| Final `AdapterEvidenceReference` assembly | `basis-producer` (implemented, Phase 2B) |
| Producer client certificate / private key custody | `basis-producer` (implemented, Phase 3) |
| Bearer subject credential custody (separate from the certificate) | `basis-producer` (implemented, Phase 3) |
...
| Certificate decoding/parsing (of the forwarded leaf certificate) | `basis-gateway` — implemented and merged, Phase 1B.1 (`auth/producer_mtls.py`) |
| Producer identity derivation (URI SAN) | `basis-gateway` — implemented and merged, Phase 1B.1 (`auth/producer_mtls.py`) |
| Producer admission (exact match) | `basis-gateway` — implemented and merged, Phase 1B.1 (`auth/producer_mtls.py`) |
| Producer trust classification (mTLS path) | `basis-gateway` — implemented and merged, Phase 1B.3 (`auth/operation_producer_mtls.py`), additive to existing |
...
Phase 1A itself (proving the topology) is complete, with Outcome B recorded above. **Phase 1B (implementing admission logic against the now-specified trusted-proxy topology) is complete** — 1B.1, 1B.2, and 1B.3 have merged, and the demonstration this gate requires has been made for the bounded, approved scope: a trusted NGINX ingress proven in CI, certificate-derived URI SAN identity, exact admission matching, and the producer workload independently authenticated from the bearer authorization subject. The reference slice does not attempt to support every deployment topology — one topology is selected, proven, and documented as this slice's own (per ADR-0009 and its companion architecture document), with permanent production ingress architecture left for later. Gateway-side Phase 1B completion was originally a distinct fact from whether the operation-producer runtime that would call this boundary existed — at the time this gate was written, it did not (§5, §30). That producer runtime now exists as `basis-producer` ([ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md), Accepted), and its bounded Phase 2A through Phase 5 implementation is complete (§26). Phase 1B and the producer's Phase 2A–5 program did not land simultaneously — Phase 1B's own completion, recorded above, preceded and was independent of the producer program that later completed against it.
...
ADR-0008 path (new)
    validated producer client certificate
        (validated by the trusted mTLS ingress ADR-0009 defines —
...
- client certificate + private key, presented to the trusted NGINX ingress ADR-0009 defines (`httpx.Client(cert=(certfile, keyfile))`, an already-supported `httpx` capability requiring no new dependency beyond `httpx` itself, which is not a `basis-adapters` dependency and was added, as planned, to `basis-producer` only in Phase 3 — never to `basis-adapters`); the producer's own TLS handshake is with the ingress, not directly with `basis-gateway`, under that topology;
...
| Evidence store interface (blob retention + reference binding) | New repository-local implementation detail | Lives entirely inside `basis-producer` (implemented, Phase 2A/2B); no other component calls it |
...
- **Outcome B occurred** (§11): implementation stopped before any producer-admission code was written, and this planning document was updated first, via [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) and [`producer-mtls-proxy-trust-boundary.md`](producer-mtls-proxy-trust-boundary.md). The proxy fallback was built only once ADR-0009 was formally accepted, and the proxy-to-gateway trust channel (a Unix domain socket, per the architecture document's §9) must be verified, by an explicit test, to be unreachable from anywhere except the proxy itself before any header-based identity assertion is trusted (architecture document Appendix B). The internal header this topology uses is `X-BASIS-Producer-Client-Cert`, defined and scoped by that document — not an ad hoc `X-Client-Cert`/`X-SSL-Client-*`-style shortcut.
...
A deterministic, offline demo/test environment, structured analogously to `basis-gateway`'s existing `demo/operation-aware/` (README, a run script, checked-in policy/expected fixtures) but living inside `basis-producer` and driving both `basis-adapters` and a locally-configured `basis-gateway` instance — Phase 5 work, now built and merged (`demo/bounded-slice/`, `basis-foundation/basis-producer` PR #5):
...
| `httpx` (mTLS-capable HTTP client) | `basis-producer` | stdlib `http.client` supports client certificates but with a materially worse ergonomic and testing story; `httpx` is already the ecosystem's chosen pattern (`basis-console`'s `GatewayClient` already uses it) | Runtime | Reused pattern, not a shared code dependency — the producer does not import `basis-console`; added in Phase 3 (`pyproject.toml` `httpx>=0.27.0`, matching `basis-gateway`'s own minimum) |
| `basis-adapters==0.2.0` (as a library dependency) | `basis-producer` | This is the entire point of the slice — reuse, not reimplementation, of normalization and evidence construction | Runtime | Added by Phase 4, merged: `pyproject.toml` now declares `basis-adapters==0.2.0` as a runtime dependency (exact-pinned, matching `basis-adapters`' own pre-v1, no-semver-guarantee convention for its one runtime dependency), so `RestAdapter.normalize()` and `construct_adapter_evidence()` can be called directly. Dependency direction remains one-way — `basis-producer` → `basis-adapters` — with no reverse or circular dependency; `basis-adapters` does not depend on, or import from, `basis-producer` |
| `cryptography` (test-fixture certificate generation) | `basis-producer` | Needed to generate the local PKI fixtures (§22) deterministically without external tooling | Test-only | Same library the gateway already depends on; no new maintenance burden ecosystem-wide; added in Phase 3 as a dev/test-only dependency (`pyproject.toml` `cryptography>=42.0.0`, used only by `tests/test_gateway_client_tls.py`'s local TLS fixtures) |
| `PyJWT` (issue the local BASIS-local identity token for the bearer-subject credential) | `basis-producer` | The subject credential must be signed offline; the gateway's own `demo/operation-aware/run_demo.py` already uses exactly this library for exactly this purpose | Test/demo-only | Already an established ecosystem library (`basis-identity` runtime dependency, `basis-gateway` demo dependency); the producer *verifies* nothing — the gateway does; added in Phase 3 as a dev/test-only dependency (`pyproject.toml` `PyJWT[crypto]>=2.8.0`, used only by `tests/test_gateway_client_live_gateway.py`'s locally issued test tokens) |
...
### Phase 2 — Reference evidence store and reference assembly (`basis-producer` repository, per ADR-0010) — Phase 2A COMPLETE, Phase 2B COMPLETE
...
#### Phase 2B — Reference lifecycle and evidence-reference assembly — COMPLETE

Second branch: `feature/phase-2b-reference-lifecycle`, merged to `main` in `basis-foundation/basis-producer` (PR #2). Began only once Phase 2A's retained-evidence foundation existed, per the retain-before-mint ordering this document requires.

1. Post-retention `reference_id` minting via injectable factory (§9, §10) — never before confirmed blob retention (Phase 2A). — Done (`generate_reference_id`, canonical-UUIDv4 validation, `src/basis_producer/reference_id.py`).
2. Reference binding: durable, confirmed `refs/<reference_id>` → digest/storage-key record, using the same flush/fsync/atomic-rename discipline as the blob (§9), published with a genuinely atomic no-clobber primitive (`os.link()`, not `os.replace()`). — Done (`FilesystemReferenceBindingStore`, `src/basis_producer/reference_binding.py`); the write-once/no-clobber concurrency guarantee is proven by a real two-thread `os.link()` race test, not merely asserted.
3. `adapter_source`/`redaction_classification` assignment (§10) — `redaction_classification` fixed to `reference_only` for this bounded phase, not caller-selectable. — Done.
4. Final `AdapterEvidenceReference` assembly, validated against `basis-core`'s existing Pydantic model, gated on confirmed binding. — Done (`EvidenceReferenceLifecycle.create_reference()`, `src/basis_producer/evidence_reference_lifecycle.py`).
5. Resolution by `reference_id` (binding → digest/storage key → blob → digest recomputation), extending Phase 2A's direct-by-digest verification to the full reference-layer chain (§9); the expected digest used for verification is sourced from the durable binding, never from the bytes under verification. — Done (`resolve_reference_evidence()`).
6. Unit tests for all of the above, including binding-failure/no-assembly and the retain-before-mint ordering itself as a mechanically enforced invariant, not merely a documented convention. — Done.

Consistent with this phase's scope, the merged implementation does not implement gateway networking, mTLS client behavior, bearer-subject credential handling, adapter orchestration, or protocol execution — those remain Phase 3 and later. This is the same retain-before-mint invariant Phase 2 always required, unchanged: storage semantics, identifier ownership, redaction semantics, and failure semantics were not altered by the 2A/2B split — only their sequencing into two PRs was. See `basis-producer`'s own [`docs/reference-lifecycle.md`](https://github.com/basis-foundation/basis-producer/blob/main/docs/reference-lifecycle.md) for the full implementation-level ordering, durability, and failure-semantics detail this section summarizes.

### Phase 3 — Reference producer gateway client — COMPLETE

Branch: `feature/phase-3-authenticated-gateway-client`, merged to `main` in `basis-foundation/basis-producer` (PR #3).

1. mTLS producer credential: `httpx` client certificate/key configuration (§15). — Done (`GatewayClientConfig`, `src/basis_producer/gateway_client_config.py`).
2. **Separate** bearer subject credential: independently configured `Authorization: Bearer` header (§14.4, §15). — Done; both credentials configured through independent settings, never defaulted from one another, and validated as both-present before any network use.
3. Operation-aware request submission against the existing, unchanged endpoint. — Done (`GatewayClient.submit()`, `src/basis_producer/gateway_client.py`; `OperationAwareSubmission`, `src/basis_producer/gateway_client_request.py` — a producer-local, field-for-field mirror of the gateway's public `OperationAwareEvaluateRequest`, not a new ecosystem schema).
4. Deterministic error handling across the layers of §17. — Done (`src/basis_producer/gateway_client_errors.py`: distinguishable configuration/connection/TLS/timeout/HTTP-rejection/protocol-failure error classes; governed `ALLOW`/`DENY`/`NOT_APPLICABLE`/evaluation-failure results returned, never raised, as normal values via `src/basis_producer/gateway_client_response.py`).
5. Tests against a locally running (or in-process test-harness) `basis-gateway` instance with Phase 1B's admission logic enabled. — Done: real local mTLS socket tests (`tests/test_gateway_client_tls.py`), real in-process gateway integration tests (`tests/test_gateway_client_live_gateway.py`, skipped automatically when `basis-gateway` is not installed), and CI execution of the live-gateway tests.

Consistent with this phase's scope, the merged implementation submits an already-composed `OperationAwareSubmission` — it does not invoke `RestAdapter.normalize()`, does not call `construct_adapter_evidence()`, and does not orchestrate `basis-adapters` or Phase 2A/2B into one pipeline from a live protocol operation. It also does not import `basis_adapters` (`pyproject.toml` runtime dependencies remain `basis-core`, `pydantic`, and `httpx` only). Those remain Phase 4.

### Phase 4 — REST vertical slice — COMPLETE

Branch: `feature/phase-4-rest-adapter-composition`, merged to `main` in `basis-foundation/basis-producer` (PR #4).

1. Wire `RestAdapter.normalize()` (§8's fixture operation) into the reference producer's Phase 2/3 pipeline end to end: adapter → evidence → digest-addressed storage → reference binding → reference assembly → dual authentication → admission → gateway → core → audit. — Done (`RestOperationComposer.submit()`, `src/basis_producer/rest_composition.py`).
2. No execution code added anywhere. — Done; pinned by `tests/test_rest_composition.py::TestNoExecution`, which also proves the module imports no network library beyond the existing Phase 3 `GatewayClient`.
3. Cross-repository integration tests (§23's positive/negative paths), including the required `producer URI SAN != bearer subject_id` assertion. — Done (`tests/test_rest_composition_trusted_ingress.py`, `tests/test_rest_composition_live_gateway.py`, run against a real trusted NGINX ingress and a real in-process/live gateway).

Consistent with this phase's scope, the merged implementation reuses `RestAdapter.normalize()`, `construct_adapter_evidence()`, `EvidenceReferenceLifecycle`, and `GatewayClient` exactly as each already existed — it adds only the composition calling them in order and stopping at the first failure; it does not reimplement any of their owned behavior and does not dispatch to the normalized REST target itself.

### Phase 5 — Cross-repository conformance, demo, and review — COMPLETE

Branch: `feature/phase-5-bounded-slice-conformance`, merged to `main` in `basis-foundation/basis-producer` (PR #5).

1. Local demonstration, positive and negative scenarios (§24). — Done (`demo/bounded-slice/run_demo.py`, four scenarios: `allow`, `deny`, `invalid-bearer`, `unadmitted-producer`; every summary prints an explicit `executed: no` line).
2. Evidence inspection and digest-verification demonstration (§24). — Done, including independent digest verification and dual-identity (producer URI SAN vs. bearer subject) narration in the demo output.
3. Trust-boundary verification — the selected topology's identity boundary is demonstrated, not asserted (§11, §21). — Done, via an eight-category conformance workflow (`demo/bounded-slice/conformance.py`): `adapter-normalization`, `evidence-integrity`, `reference-lifecycle`, `producer-mtls-admission`, `bearer-subject-independence`, `deterministic-allow`, `deterministic-deny`, `no-execution` — each category delegating to the real, already-merged test suite rather than introducing a parallel one.
4. Documentation of what the slice proves and does not prove (§28). — Done (`docs/release-readiness.md`).
5. Release-readiness assessment per repository (§27). — Done: `docs/release-readiness.md` concludes the repository is ready for an initial, explicitly bounded, pre-v1 release of its authorization-only scope (no release tag has been cut by this plan); no release was found required for `basis-adapters`, `basis-core`, or `basis-schemas`.
6. The permanent-repository maturity review (§30) — repository *existence* is settled by ADR-0010; §30's maturity checklist was evaluated by the Phase 5 release-readiness review, which found the bounded slice ready for broader responsibility within its explicitly bounded, authorization-only scope, and found no BLOCKER.

This plan's five-phase implementation PR sequence is now complete. No Phase 6 is defined by this plan: the bounded reference slice stops at the authorization disposition, and extending it into protocol execution is a separate, future architecture decision this plan does not make.

Phases are ordered to minimize rework and to prevent building on an unproven boundary: Phase 1A is a gate, not a task; gateway admission logic (Phase 1B) does not depend on the reference producer existing and can be fully tested against synthetic client certificates before any producer code is written; the evidence store (Phase 2A) does not depend on gateway changes at all; Phase 2B depends only on Phase 2A, not on the gateway; only Phase 3 requires both the producer's own Phase 2 output and the gateway to exist simultaneously.
...
| `basis-producer` | New, entirely internal to itself | No formal release/package-index publication for this reference/demo-scoped component (§5) |
...
- ~~Permanent operation-producer-runtime repository placement~~ (§30) — **resolved:** [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) establishes `basis-producer` as the answer and is recorded `Accepted`. The repository now exists and all five implementation phases (Phase 1B, 2A, 2B, 3, 4, 5) are implemented and merged; §30's maturity checklist was evaluated by the Phase 5 release-readiness review — see §30.
...
| Post-retention `reference_id` minting | Yes — merged | `basis-producer` | **Phase 2B — complete** |
| `reference_id` → digest/storage-key binding record | Yes — merged | `basis-producer` | **Phase 2B — complete** |
| Resolution by `reference_id` (binding → digest/storage key → blob) | Yes — merged | `basis-producer` | **Phase 2B — complete** |
| `adapter_source` / `redaction_classification` assignment | Yes — merged | `basis-producer` | **Phase 2B — complete** |
| `AdapterEvidenceReference` assembly | Yes — merged | `basis-producer` | **Phase 2B — complete** |
| mTLS client presentation | Yes — merged | `basis-producer` | **Phase 3 — complete** |
| Separate bearer-subject credential presentation | Yes — merged | `basis-producer` | **Phase 3 — complete** |
| Gateway submission | Yes — merged | `basis-producer` | **Phase 3 — complete** |
...
| End-to-end REST-to-disposition integration | Yes — merged | cross-repository | **Phase 4 — complete** |
| Local demo / conformance evidence | Yes — merged | `basis-producer` | **Phase 5 — complete** |
