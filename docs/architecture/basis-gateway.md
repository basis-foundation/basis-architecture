...
**basis-identity** — The identity engine is the federation and normalization boundary that sits upstream of the gateway. It integrates external, authoritative identity providers (Keycloak, Okta, Entra ID, Auth0, Ping, ADFS, LDAP, and other SAML/OIDC sources), owns the login/logout/callback flows and BASIS-local sessions that federation requires, maps provider claims, resolves subjects, and produces canonical BASIS identity context — issuing BASIS-local tokens when a deployment is configured for it. The gateway validates the trusted identity context or BASIS-local token that `basis-identity` produces; it does not re-run federation or claim mapping. In deployments where `basis-identity` is present, the gateway's authentication responsibility narrows to validating that trusted context/token rather than integrating each upstream provider directly. In deployments without `basis-identity`, the gateway verifies external identity directly (the model described in the Authentication Model section below). In both cases the gateway, not the identity engine, owns enforcement: validating the request, composing the `DecisionRequest`, invoking `basis-core`, and emitting gateway audit evidence. See [`docs/architecture/basis-identity.md`](basis-identity.md) for the canonical identity engine architecture reference.
...
**Relationship to basis-identity.** The authentication and normalization model described above is the gateway's behavior when it verifies external identity directly. When `basis-identity` — the BASIS identity engine and federation boundary — is deployed in front of the gateway, federation, login/logout/callback flows, session management, and claim mapping move to `basis-identity`, which presents the gateway with trusted canonical identity context or a BASIS-local token. In that model the gateway validates that trusted context/token rather than integrating each upstream provider itself; the normalization-into-`Subject`/`IdentityContext` contract the kernel depends on is unchanged. Both models are architecturally supported: direct verification at the gateway, or federation upstream at `basis-identity` with the gateway validating its output. See [`docs/architecture/basis-identity.md`](basis-identity.md).

**What the gateway does not own.** The gateway verifies credentials; it does not issue them. It does not operate an identity provider, manage LDAP connections, administer Keycloak realms, or participate in certificate authority operations. Integrating external identity providers and brokering federation belong to `basis-identity`; the authoritative provider operation belongs to the external IdPs themselves. Those responsibilities belong to `basis-identity`, deployment infrastructure, and, in commercial deployments, to BASAuth managed services.
