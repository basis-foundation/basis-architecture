...
This document does not assign these phases to a specific kernel subpackage. [`docs/architecture/operation-aware-evaluation-orchestration.md`](operation-aware-evaluation-orchestration.md) and [ADR-0006](../adr/0006-evaluation-orchestration-layer.md) name the `evaluation` subpackage's future `engine` module as the orchestrator of this sequence — invoking `policy`-owned evaluation logic for steps 4–6 and composing the resulting facts into the trace and response contracts steps 8–9 describe — without redefining the semantics stated here.
