...
**Status:** Architecture-planning, with the bounded producer-authorization portion of the boundary now implemented. This document defines logical roles, trust rules, correlation semantics, and a decision-gated implementation sequence for the next unresolved boundary in the identity-to-operation chain. It does not implement runtime behavior in any repository and does not add or modify a schema. This document originally defined the producer/execution boundary before a producer workload-authentication technology had been selected. [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) subsequently selected mutual TLS as the first normative producer-to-gateway workload-authentication profile — Accepted. [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — Accepted — defined the bounded trusted-ingress topology required to implement that accepted mTLS profile; `basis-gateway`'s Phase 1B (1B.1, 1B.2, and 1B.3) has since merged, so gateway-side producer mTLS authentication and admission — trusted NGINX ingress, certificate-derived URI SAN identity, exact admission matching, and dual authentication of the producer workload independent of the bearer authorization subject — is implemented and proven for its bounded, approved scope. The operation-producer runtime itself exists as `basis-producer`, and its evidence-retention responsibility (Phase 2A), its reference-lifecycle responsibility (Phase 2B), its gateway-client responsibility (Phase 3), its REST adapter composition (Phase 4), and its cross-repository conformance/demonstration/release-readiness review (Phase 5) are all implemented and merged. §11's own repository-placement decision gate is resolved by [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) — `basis-producer` is established as the permanent operation-producer-runtime repository and component, with Phase 1B's completed, proven gateway-side boundary as additional evidence that the trust boundary the producer sits behind is real rather than speculative; ADR-0010 is recorded `Accepted`. **The adapter → producer → gateway authorization boundary this document names is therefore implemented for its bounded REST reference scope: a real, conformance-tested path exists from `RestAdapter.normalize()` through evidence construction, retention, reference assembly, dual authentication, trusted-ingress admission, and gateway/kernel evaluation to an authoritative disposition.** The second gap this document names — gateway authorization → execution → execution evidence — remains entirely unresolved and unimplemented; nothing below should be read as claiming otherwise. Completion of the bounded producer-authorization slice does not itself authorize or define the next execution architecture; that is a separate, not-yet-made architecture decision. [`docs/architecture/execution-boundary-discovery-assessment.md`](execution-boundary-discovery-assessment.md) is the active, non-normative discovery assessment of that second gap: an evidence-based current-state inventory, ownership and topology analysis, and decision inventory that this document's own Non-Goals and Open Questions Deferred sections anticipated but did not themselves resolve. It does not supersede this document and does not authorize execution implementation. [ADR-0011](../adr/0011-protocol-execution-role-and-bounded-reference-topology.md) — `Proposed`, not `Accepted` — is the first normative decision addressing that second gap: it establishes the protocol-executor role named below (§2) as architecturally distinct from the operation-producer role and selects same-process colocation with `basis-producer` as the default topology for the first bounded execution reference slice, without selecting an authorization-to-execution binding mechanism, an execution-lifecycle vocabulary, or an execution-evidence contract, and without creating `basis-executor` or any other new repository. Acceptance of ADR-0011 would not itself authorize execution implementation; its own follow-on decision gates remain outstanding. This document's own role definitions (§2) and lifecycle invariants (§5) are the vocabulary ADR-0011 inherits rather than redefines.
...
Between those two, well-specified components sits a gap: **how a `NormalizedAuthorizationRequest` produced by `basis-adapters` actually becomes an authenticated, trusted-producer request accepted by `basis-gateway`.** At the time this document was originally authored, BASIS had not yet defined how that crossing would happen at all. Subsequent architecture work has resolved the bounded producer-authentication portion of that problem: [ADR-0007](../adr/0007-adapter-evidence-construction.md) established evidence-construction ownership, [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — Accepted — selected mTLS and gateway-owned producer-admission semantics, [`bounded-operation-producer-reference-implementation-plan.md`](bounded-operation-producer-reference-implementation-plan.md) defined a phased integration path, and [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — Accepted — defines the trusted-ingress certificate-handoff topology that plan's Phase 1A required after reaching Outcome B. Nothing in `basis-adapters` authenticates to the gateway, by design — that separation is unchanged. The two contracts are compatible in shape — the `AdapterEvidenceReference` model that `basis-gateway`'s operation-aware request accepts is structurally built to reference the evidence `basis-adapters` preserves. **This handoff is now implemented for its bounded REST reference scope.** The operation-producer runtime, `basis-producer`, exists and has implemented evidence retention (Phase 2A), reference lifecycle (Phase 2B), the authenticated gateway client (Phase 3), and — composing that client with real `RestAdapter.normalize()` output — REST adapter composition (Phase 4); `basis-producer`'s Phase 5 conformance, demonstration, and release-readiness review is likewise implemented and merged. The complete handoff this section names — adapter normalization through authenticated, admitted, evidence-referenced gateway submission — is therefore proven end to end for one bounded REST reference path, not merely specified.
...
Both gaps share a root cause: the ecosystem has fully specified the components on either side of each gap (adapters normalize; the gateway authenticates, classifies, and enforces; the kernel decides) but, at the time this document was originally authored, had not yet specified what runs *between* normalization and gateway ingestion, or what runs *after* gateway enforcement and before a device sees the operation. This document names those missing logical roles, states the trust and provenance rules that govern them, and sequences the decision-gated work required to fill them — without assigning that work to an implementation repository prematurely and without inventing runtime behavior this document is not authorized to add. On the producer-ingestion side, that role is now specified as the operation-producer runtime (§2), its authentication mechanism is Accepted (ADR-0008), and Accepted ADR-0009 defines the trusted-ingress topology that implementation requires. **That implementation is now complete for its bounded, approved scope:** the gateway-side trust boundary (Phase 1B) and `basis-producer`'s Phase 2A evidence retention, Phase 2B reference lifecycle, Phase 3 authenticated gateway submission, Phase 4 REST adapter composition, and Phase 5 conformance/demonstration/release-readiness review are all implemented and merged — the bounded operation-producer authorization reference slice (§13 Stage 5) is complete. On the execution side, the protocol executor and execution-evidence producer roles remain named but genuinely unspecified beyond §2's definitions: no authentication, transport, or implementation decision has been made for either, and none is made by this reconciliation.
...
This document calls the logical role the *operation producer* (or, when referring to the running software, the *operation-producer runtime*) — vocabulary this document originally introduced as working terminology, consistent with how both `docs/roadmaps/identity-to-operation-contract-and-interoperability.md` and `docs/roadmaps/post-authentication-identity-activity-correlation-and-detection.md` introduce their own working vocabulary. That naming question is no longer open: [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) — Accepted — permanently establishes `basis-producer` as the Foundation-maintained implementation of this role, with `basis-foundation/basis-producer` as its repository. The role itself remains conceptually distinct from that implementation — other conforming implementations remain architecturally possible — but the placement/naming decision §11 discusses below is resolved, not merely evaluated. The `basis-producer` repository now exists, and its Phase 2A evidence-retention foundation, Phase 2B reference-lifecycle responsibility, Phase 3 authenticated gateway-submission responsibility, and Phase 4 adapter orchestration/composition are all implemented and merged, with Phase 5's cross-repository conformance, demonstration, and release-readiness review likewise complete; protocol execution remains unimplemented.

### Gateway

The existing `basis-gateway` responsibility, unchanged by this document. It authenticates the producer runtime and classifies whether the authenticated caller is a trusted operation producer. Two coexisting mechanisms now establish that classification: `OPERATION_PRODUCER_SUBJECT_IDS` (exact-match, case-sensitive, empty by default — see §3), retained as the legacy compatibility path used when trusted-proxy mTLS mode is disabled; and, per ADR-0008/ADR-0009 and `basis-gateway`'s merged Phase 1B (1B.1–1B.3), the bounded normative mTLS path — trusted-ingress certificate handoff, URI-SAN-derived producer identity, and exact admission matching — used when trusted-proxy mode is enabled, which never falls back to the legacy allowlist. Authorization-subject authentication remains independently bearer-based in either case. It accepts or rejects producer-only context accordingly; records every accepted field's provenance; invokes `basis-core`; enforces the returned disposition at its own HTTP boundary; and records `GatewayAuditEvent` beside the kernel's unmodified `AuditEvidence`. It does not parse OT protocols, does not independently verify a producer's protocol claims, and — per its own documented limitation — does not claim that device execution occurred.
...
Trust in an operation producer is not one fact. This document distinguishes seven separate facts that are routinely conflated when discussing "trusted producers," because conflating them is exactly the failure mode the ecosystem's threat model already names for the existing, narrower gateway↔adapter trust boundary (`docs/security/threat-model.md` §3.3, §4.2, §6.3 — rogue integrator and compromised-adapter adversaries; "an adapter receiving untrusted protocol input does not treat the protocol channel as evidence of authorization"). Two of these facts — producer workload identity and authorization-subject identity — were originally described together as a single "authenticated workload identity" fact, before ADR-0008 established them as structurally and semantically separate; they are listed as distinct rows below to match the implemented dual-identity architecture.

| Distinct fact | What it answers | Where it is established today |
| - | - | - |
| Producer workload identity | Which software process — the operation-producer workload itself — is submitting this call, independent of any authorization-subject identity? | Validated mTLS client certificate → `basis_gateway.auth.producer_mtls`'s exactly-one-eligible-URI-SAN derivation, taken verbatim from the certificate's URI SAN (never Common Name, DNS SAN, IP SAN, or email SAN) — ADR-0008; `basis-gateway` Phase 1B, implemented |
| Authenticated authorization-subject identity | Which bearer-authenticated subject's authority does this request act under? | `basis-gateway`'s existing `authenticate()` dispatch (OIDC or `basis_local_token`), presented independently of and never derived from the producer's own mTLS workload credential (ADR-0008; `basis-producer` Phase 3, implemented) |
| Authorization to act as an operation producer | Is this authenticated producer workload allowed to submit operation-aware requests at all, and specifically to assert producer-only context? | Two mutually exclusive paths, selected per deployment by `OPERATION_PRODUCER_MTLS_TRUSTED_PROXY_ENABLED`: the implemented mTLS trusted-proxy admission path (`OPERATION_PRODUCER_MTLS_ADMITTED_URIS`, exact-match against the certificate-derived URI SAN — ADR-0008, ADR-0009; `basis-gateway` Phase 1B, implemented), or the retained legacy `OPERATION_PRODUCER_SUBJECT_IDS` compatibility allowlist, exact-match against the bearer-authenticated `subject_id`; the two allowlists are distinct trust mechanisms, never merged, compared, or consulted together for a single request |
...
Several rules follow directly from keeping these six facts distinct, and are already implemented, in narrower form, by `basis-gateway`'s existing producer-trust module — this document generalizes them to the operation-producer runtime, which now sits on the other side of an implemented, proven producer-to-gateway path (§3, §4) rather than a hypothetical future one:

- **Producer trust must not be self-asserted in a request body.** `basis-gateway`'s operation-aware schema already enforces this for its own callers by rejecting `is_trusted_operation_producer` / `producer_trust_classification` as caller-supplied fields; the same rule holds at the now-implemented adapter-to-gateway authentication mechanism this document originally left unselected — ADR-0008 selected mTLS, ADR-0009 selected the trusted-ingress topology, and `basis-gateway`'s Phase 1B implements both — and must continue to hold at any future extension of it.
...
- **The current gateway subject-ID allowlist is a valid, bounded first implementation — not the final ecosystem trust model.** `ROADMAP.md`, at the time this document was originally written, described `OPERATION_PRODUCER_SUBJECT_IDS` as "not the adapter-to-gateway trusted-producer contract... which remains open." This document itself did not close that gap; it named the shape of the decision (§13, Stage 4) without selecting a transport-authentication technology, per its own explicit non-goals at the time. **That gap is no longer open as an authentication-mechanism question, and the bounded reference implementation of it is now complete.** [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — Accepted — selected mutual TLS as the first normative producer-to-gateway workload-authentication profile and retains the allowlist as a compatibility mechanism rather than removing it. `basis-gateway`'s Phase 1A spike found direct in-process mTLS termination unsafe (Outcome B), [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — Accepted — defined the trusted-ingress topology required to carry a validated certificate into gateway application code, and `basis-gateway`'s Phase 1B (1B.1, 1B.2, and 1B.3) has since merged: gateway-side producer authentication and admission against that topology is implemented and proven. The operation-producer runtime, `basis-producer`, now exists and has implemented evidence retention (Phase 2A), reference lifecycle (Phase 2B), gateway submission (Phase 3), and — composing that authenticated submission from a real, adapter-normalized operation — REST adapter composition (Phase 4); its Phase 5 conformance, demonstration, and release-readiness review is likewise implemented and merged. Category-scoped producer capability (the granularity question in the paragraph below) remains open independent of the authentication-mechanism question, and is not resolved by either ADR or by the now-complete bounded slice.
...
The governed handoff from normalization to gateway submission has four stages, and all four are now chained together in one live, conformance-tested path: `basis-adapters`' normalization and evidence-material surface (`RestAdapter.normalize()`, `protocol_evidence` construction) is implemented; `basis-producer`'s producer-identity binding and evidence-reference assembly mechanism (Phase 2A, Phase 2B) is implemented; `basis-producer`'s authenticated gateway submission (Phase 3) is implemented; `basis-producer`'s composition of all of the above, driven by a real adapter-normalized operation rather than a manually assembled submission (Phase 4), is implemented; and `basis-gateway`'s downstream operation-aware request handling into `basis-core` is implemented:

```text
ProtocolOperation
    ↓  (basis-adapters: implemented)
NormalizedAuthorizationRequest  (+ protocol_evidence, preserved verbatim)
    ↓  (operation-producer runtime: implemented end to end —
       Phase 2A, Phase 2B, Phase 3, and Phase 4 composition
       from real adapter-normalized output)
...
composes producer-only context it is authorized to assert
    ↓  mTLS producer workload authentication (ADR-0008, Accepted;
       basis-producer Phase 3, implemented)
    ↓  trusted ingress topology governed by ADR-0009
       (Accepted; basis-gateway Phase 1B — 1B.1, 1B.2, 1B.3 — merged
       and implements this step for its bounded, approved scope)
operation-aware gateway request
    (POST /v1/evaluate/operation-aware, basis-gateway: implemented,
     feature-flagged, disabled by default)
```

This diagram originally showed the operation-producer-runtime-to-`basis-gateway` step as "authentication mechanism not yet selected." That was accurate when this document was first authored; it is no longer current. [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) has since selected mutual TLS as the first normative producer workload-authentication profile, and [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — Accepted — defines the trusted-ingress topology needed to carry an authenticated producer certificate into `basis-gateway` application code given that direct in-process termination proved unsafe (§13 Stage 4's sub-gate, below). `basis-gateway`'s Phase 1B has since merged in full (1B.1, 1B.2, 1B.3), so this step — trusted-ingress certificate handoff, URI SAN derivation, and exact producer admission — is implemented and gateway-side complete; the operation-producer runtime's own responsibility (the second stage above) is complete as well: `basis-producer` exists and has implemented evidence retention, reference lifecycle, the authenticated gateway client, and adapter orchestration — composing that mechanism from `RestAdapter.normalize()`'s actual output rather than a manually assembled submission (Phase 2A, Phase 2B, Phase 3, Phase 4, all merged). Neither ADR changes the operation-aware gateway request itself, which remains implemented and unchanged, and neither authorizes any protocol execution step beyond this diagram — this diagram still stops at the gateway request; nothing downstream of it (execution) is implemented.

The second and third stages were the producer-side gap this document originally identified. They remain conceptually distinct responsibilities even though the bounded REST reference implementation now composes them end to end: binding workload identity to a request is a distinct act from constructing a safe evidence reference, and both are distinct from actually submitting the HTTP request under gateway-recognized authentication — `basis-producer`'s Phase 2A/Phase 2B, and Phase 3, implement each in turn, and Phase 4 composes all of it from a real, adapter-normalized operation.
...
| - | - | - |
| `action`, `resource_type`, `resource_id` | `basis-adapters` produces the normalized bare action verb and resource fields; `basis-producer` carries those governed fields into the existing Phase 3 submission shape unchanged, inventing no canonical action/resource semantics; `basis-gateway` alone composes them into kernel-canonical form before invoking `basis-core` | Existing action/resource-composition contracts (`ecosystem-contract-inventory.md` §3.3, §3.5) apply unchanged; this document does not add a new composition rule |
...
| - | - | - |
| Producer workload identity | Trusted NGINX mTLS ingress (certificate validation) → `basis_gateway.auth.producer_mtls`'s exactly-one-eligible-URI-SAN derivation (gateway-owned) → `OPERATION_PRODUCER_SUBJECT_IDS` exact-match admission (gateway-owned) — ADR-0008, ADR-0009; `basis-gateway` Phase 1B, implemented | Validated by the mTLS client certificate at the trusted ingress, derived deterministically from the certificate's URI SAN (never Common Name, DNS SAN, IP SAN, or email SAN), and admitted only on an exact configured match; structurally distinct from, and never derived from or conflated with, the independently bearer-authenticated authorization subject below |
| Authenticated authorization-subject identity | Authentication boundary (today: `basis-gateway`'s `authenticate()` — OIDC or `basis_local_token`; future: whatever authenticates it) | Verified according to the configured authentication mode; not self-asserted; presented independently of and never derived from the producer's own mTLS workload credential |
...
basis-producer
    Foundation-maintained implementation of the operation-producer
    runtime role (§2): evidence retention, reference-lifecycle
    management, producer workload credential custody, authenticated
    gateway submission, and REST adapter composition
    (established by ADR-0010, Accepted; repository exists —
    Phase 2A evidence retention, Phase 2B reference lifecycle,
    Phase 3 authenticated gateway client, Phase 4 REST adapter
    composition, and Phase 5 conformance/demo/release-readiness
    review are all implemented and merged: the bounded
    authorization slice is complete — the logical operation-producer
    role itself remains distinct from this named implementation; see
    §11. Protocol execution is unimplemented, execution-result
    evidence is unimplemented, and this is not a complete production
    runtime)
...
- `basis-adapters` does not automatically expand into a daemon, gateway, or device controller merely because the ecosystem now names an operation-producer runtime and a protocol executor. The operation-producer runtime's permanent home is now `basis-producer` (§11, ADR-0010, Accepted); the protocol executor role remains undecided. Neither role is, or becomes, in scope for `basis-adapters` per its own non-responsibilities.
...
**Direction, resolved:** preserve `basis-adapters` as the normalization library, unchanged, and define a separate operation-producer runtime role (alternative 2). This document originally treated "does the operation-producer runtime need its own repository" as an explicit, later decision gate (§13, Stage 5), to be resolved once a bounded reference implementation existed to learn from. [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) resolved that gate ahead of the bounded slice's completion: `basis-producer` is established as the permanent repository and component for the operation-producer runtime role, reasoning that `basis-gateway`'s Phase 1A/1B mTLS and admission topology (ADR-0008, ADR-0009) had already fixed the shape of the trust boundary the role sits behind, independent of whether the bounded evidence/reference slice itself had finished. ADR-0010 is recorded `Accepted`, following independent formal-acceptance review, consistent with `ROADMAP.md`'s own stated principle that "`basis-adapters` and `basis-identity` should be changed based on real integration needs that gateway work surfaces, not speculative taxonomy work performed ahead of it" — the principle ADR-0010 itself relies on in finding the gateway-side integration need real rather than speculative. This section's decision gate is resolved, and the bounded slice's own implementation and maturity review (bounded plan §30) are both now complete as well — see the plan document's own current status.
...
| Producer admission/capability onboarding contract | Operation-producer runtime | `basis-gateway` | The authentication mechanism itself is no longer unresolved: ADR-0008 selected mutual TLS, ADR-0009 defined the trusted-ingress topology, and `basis-gateway` Phase 1B and `basis-producer` Phase 3 both implement it (§3, §11). What remains is narrower: admission is deployment-owned configuration (`OPERATION_PRODUCER_MTLS_ADMITTED_URIS`, or the retained legacy `OPERATION_PRODUCER_SUBJECT_IDS`), not an interoperable, machine-readable descriptor of a producer's identity and granted capabilities that an independently-operated deployment could exchange or validate against. | A deployment onboarding a second, independently-operated producer runtime with no shared code to the first — today this can only be configured as an opaque URI SAN or subject-ID string, not described in an interoperable, machine-readable onboarding contract. |
...
| Workload/machine identity as a first-class identity concept | `basis-identity` | `basis-gateway`'s existing bounded mTLS producer-authentication path | `basis-identity`'s `CanonicalSubject` has a `SubjectType.SERVICE` value but no workload-identity establishment pipeline (no client-credentials or mTLS/SPIFFE-shaped `AuthenticationProtocol` member) and no glossary entry for "workload identity." This is not a gap in whether producer workload identity is established and verified at all — `basis-gateway`'s bounded mTLS path already does both today (§3) — it is a gap in `basis-identity`'s own first-class coverage of the concept, not in `basis-schemas`, but it constrains what a future generalized workload-identity schema can assume already exists. | An operation-producer runtime whose workload identity is already established and verified today via the bounded mTLS path (§3), but that needs a first-class `basis-identity` lifecycle (issuance, rotation, revocation, a `CanonicalSubject`-native representation) rather than deployment-configured certificate/URI-SAN admission alone. |
...
This document does not conclude that a new schema is required for any of the above merely because a gap can be named. Per `ecosystem-contract-inventory.md`'s own governing principle, a contract becomes schema-ready when implementation proves a stable shape — none of the five gaps above have a reference implementation of the specific missing contract shape yet to prove one. This is a statement about the missing schema shapes themselves, not about whether the underlying capability is implemented: producer workload authentication (§3) is implemented today without a shared schema, precisely because deployment-owned mTLS/URI-SAN configuration has proven sufficient for the bounded reference slice; the gap above is the narrower question of an interoperable onboarding descriptor, not the authentication mechanism itself.
...
4. **Gateway producer-authentication and admission refinement** — `basis-gateway`-owned work to move beyond the subject-ID allowlist toward whatever authentication mechanism the architecture eventually selects for the operation-producer runtime, and to resolve or explicitly defer the category-scoped capability question from §3 and §12.
   **Decision gate:** which authentication/transport mechanism (mTLS, SPIFFE, OAuth client-credentials, or another) — explicitly not selected by this document. **Resolved and implemented at the gateway and the producer.** [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) selects mutual TLS as the first normative producer-to-gateway workload-authentication profile, distinguishes authentication from admission, and authorizes (without implementing) a bounded producer reference slice; it is recorded `Accepted`. [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — Accepted — defines the trusted-ingress topology Phase 1A found necessary (Outcome B). `basis-gateway`'s Phase 1B — 1B.1, 1B.2, and 1B.3 — has since merged in full: mTLS validation, gateway admission configuration, certificate-derived URI SAN identity, and exact admission matching are implemented and proven for their bounded, approved scope. `basis-producer` now exists and has implemented evidence retention (Phase 2A), reference lifecycle (Phase 2B), the mTLS gateway client (Phase 3) that authenticates against this now-implemented gateway boundary, and REST adapter composition (Phase 4) — composing that authenticated client with real, adapter-normalized operation output. Permanent operation-producer-runtime repository placement is no longer a deferred decision gate: [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) — Accepted — establishes `basis-producer` as that permanent repository and component (§11).
5. **Bounded producer-runtime reference implementation** — a first, narrow implementation of the operation-producer runtime role (§2), scoped to one protocol and one deployment topology (§9), used to validate that the roles and rules this document defines are actually sufficient before generalizing them.
   **Decision gate:** does this reference implementation live in an existing repository (a `basis-gateway` client library, for instance) or does it justify a new repository, per §11's deferred decision? **Resolved:** [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) establishes `basis-producer` as a new, permanent repository for this role; `Accepted`. The repository now exists, and all five phases of its implementation sequence — Phase 1B (gateway boundary), Phase 2A (evidence-retention foundation), Phase 2B (reference lifecycle), Phase 3 (authenticated gateway client), Phase 4 (REST adapter composition), and Phase 5 (cross-repository conformance, demonstration, and release-readiness review) — are all implemented and merged. **The bounded reference implementation is complete for its approved scope.** **Implementation planning:** [`docs/architecture/bounded-operation-producer-reference-implementation-plan.md`](bounded-operation-producer-reference-implementation-plan.md), authorized by [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md), defines the phased implementation sequence this reference implementation followed, targeting the permanently established `basis-producer` repository per ADR-0010; that plan's own status now records the full sequence as complete. **Producer-mTLS transport-boundary sub-gate — resolved and implemented.** That plan's own Phase 1A named a further, narrower blocking gate — where mTLS actually terminates and how a validated client certificate's identity reaches gateway application code — which `basis-gateway`'s Phase 1A spike resolved to Outcome B (direct application-level termination is not a safe boundary under the current ASGI/Uvicorn stack), requiring a trusted mTLS ingress in front of the gateway per [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md)'s mTLS profile. [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) and [`docs/architecture/producer-mtls-proxy-trust-boundary.md`](producer-mtls-proxy-trust-boundary.md) define that ingress topology and are recorded `Accepted`. `basis-gateway`'s Phase 1B — 1B.1, 1B.2, and 1B.3 — has since merged: gateway-side producer authentication and admission (trusted-ingress certificate handoff, URI SAN derivation, exact admission matching, and dual authentication of the producer workload independent of the bearer authorization subject) is implemented and proven for the bounded, approved scope this sub-gate describes. `basis-producer` implements the evidence-retention, reference-lifecycle, authenticated-gateway-client, and adapter-composition quarters of the work ADR-0007 and ADR-0008 assign to it (Phase 2A, Phase 2B, Phase 3, Phase 4, all merged) — the client holds a producer certificate and submits, driven by real adapter-normalized output, against this proven gateway boundary. This sub-gate does not change §2's role definitions, §3's trust rules, or this document's own repository-placement decision gate — it narrowed only the transport question of how an already-decided authentication mechanism reaches gateway code, consistent with this document's own non-goal of not selecting a producer-authentication mechanism.
6. **Execution-evidence contract publication, only after producer and consumer are stable** — `basis-schemas` work. Stage 5's bounded producer-runtime reference implementation is COMPLETE for its authorization-only scope, and by design produced no protocol executor and no execution evidence — execution was never in that stage's scope (Non-Goals, above). This stage is not gated on anything Stage 5 produced; it is gated on three things that do not yet exist and that this document does not create: (1) a future, separately accepted execution-boundary architecture decision — not selected here; (2) a real, bounded protocol-executor / execution-evidence-producer implementation built under that future architecture; and (3) sufficient implementation evidence from that real implementation to stabilize the execution-result shape, per `ecosystem-contract-inventory.md`'s own "implementation proves a stable shape" principle. Stage 5 was never expected to supply any of the three — this document does not design that future execution implementation.
   **Decision gate:** is the five-state sketch from the interoperability roadmap's Phase 4 sufficient, or does the reference implementation surface additional states this document did not anticipate? This remains open, and cannot be resolved until (1)–(3) above exist.
...
8. **Deployment packaging** — `basis-deploy` work, once that repository exists, to package whatever combination of roles has been validated by then — Stage 5's bounded producer-authorization role today, and, separately and only once it exists and is itself validated, the future execution-related role(s) Stages 6–7 depend on — without `basis-deploy` acquiring semantic ownership of any of them (§10). Stage 5 validated the bounded producer-authorization role only; it did not validate a protocol executor or any execution-related role, and this stage does not assume one is ready for packaging merely because Stage 5 completed.
...
- ~~Whether the operation-producer runtime requires a new repository, or can live inside an existing one as a reference client~~ (§11, §13 Stage 5) — **resolved:** [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) establishes `basis-producer` as the permanent repository and component; it is recorded `Accepted`. The repository now exists and all five phases of the bounded slice's implementation — Phase 1B, Phase 2A, Phase 2B, Phase 3, Phase 4, and Phase 5 — are implemented and merged, including the Phase 5 maturity/release-readiness review (per the implementation plan's §30). The bounded reference slice is complete for its approved scope.
- ~~Which authentication and transport mechanism establishes producer workload identity toward `basis-gateway` beyond the current subject-ID allowlist~~ (§3, §13 Stage 4) — **resolved and implemented at the gateway and the producer:** [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) selects mutual TLS as the first normative profile and is recorded `Accepted`; [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — Accepted — defines the trusted-ingress topology needed to carry the authenticated certificate into `basis-gateway` given that direct in-process termination proved unsafe (`basis-gateway` Phase 1A, Outcome B). `basis-gateway`'s Phase 1B — 1B.1, 1B.2, and 1B.3 — has since merged: gateway-side producer mTLS authentication and admission is implemented and proven for its bounded, approved scope. The operation-producer runtime, `basis-producer`, now exists and has implemented evidence retention (Phase 2A), reference lifecycle (Phase 2B), the mTLS client that authenticates against this now-implemented gateway boundary (Phase 3), and composing that authenticated client with a real, adapter-normalized operation (Phase 4). Nothing remains open in this authentication/transport question for the bounded reference slice.
