...
5. **Bounded producer-runtime reference implementation** — a first, narrow implementation of the operation-producer runtime role (§2), scoped to one protocol and one deployment topology (§9), used to validate that the roles and rules this document defines are actually sufficient before generalizing them.
   **Decision gate:** does this reference implementation live in an existing repository (a `basis-gateway` client library, for instance) or does it justify a new repository, per §11's deferred decision? **Implementation planning:** [`docs/architecture/bounded-operation-producer-reference-implementation-plan.md`](bounded-operation-producer-reference-implementation-plan.md), authorized by [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md), selects a provisional (explicitly not permanent) location for this reference implementation and a phased implementation sequence; implementation itself has not begun, and permanent repository placement remains this section's own deferred decision, unresolved by that plan. **Producer-mTLS transport-boundary sub-gate:** that plan's own Phase 1A named a further, narrower blocking gate — where mTLS actually terminates and how a validated client certificate's identity reaches gateway application code — which `basis-gateway`'s Phase 1A spike has since resolved to Outcome B (direct application-level termination is not a safe boundary under the current ASGI/Uvicorn stack) and [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md)'s mTLS profile therefore requires a trusted mTLS ingress in front of the gateway for the bounded reference deployment. [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) and [`docs/architecture/producer-mtls-proxy-trust-boundary.md`](producer-mtls-proxy-trust-boundary.md) define that ingress topology; ADR-0009 is recorded `Proposed`, not yet `Accepted`. Producer admission implementation (the remainder of Stage 5's bounded slice) remains unimplemented and now depends on ADR-0009's formal acceptance in addition to ADR-0008's. This sub-gate does not change §2's role definitions, §3's trust rules, or this document's own repository-placement decision gate — it narrows only the transport question of how an already-decided authentication mechanism reaches gateway code, consistent with this document's own non-goal of not selecting a producer-authentication mechanism.
...
- ~~Which authentication and transport mechanism establishes producer workload identity toward `basis-gateway` beyond the current subject-ID allowlist~~ (§3, §13 Stage 4) — **resolved as a mechanism question:** [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) selects mutual TLS as the first normative profile and is recorded `Accepted`. What remains open is narrower and now has its own decision gate: the trusted-ingress topology needed to carry the authenticated certificate into `basis-gateway` given that direct in-process termination proved unsafe (`basis-gateway` Phase 1A, Outcome B), which [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — Proposed, not yet accepted — now defines. Gateway producer-admission implementation (Phase 1B) remains blocked on ADR-0009's formal acceptance, not on an unselected mechanism.
