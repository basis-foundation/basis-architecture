...
**Status:** Architecture-planning. This document defines logical roles, trust rules, correlation semantics, and a decision-gated implementation sequence for the next unresolved boundary in the identity-to-operation chain. It does not implement runtime behavior in any repository and does not add or modify a schema. This document originally defined the producer/execution boundary before a producer workload-authentication technology had been selected. [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) subsequently selected mutual TLS as the first normative producer-to-gateway workload-authentication profile — Accepted. [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — Accepted — defined the bounded trusted-ingress topology required to implement that accepted mTLS profile; `basis-gateway`'s Phase 1B (1B.1, 1B.2, and 1B.3) has since merged, so gateway-side producer mTLS authentication and admission — trusted NGINX ingress, certificate-derived URI SAN identity, exact admission matching, and dual authentication of the producer workload independent of the bearer authorization subject — is implemented and proven for its bounded, approved scope. The operation-producer runtime itself exists as `basis-producer`, and its evidence-retention responsibility (Phase 2A), its reference-lifecycle responsibility (Phase 2B), and its gateway-client responsibility (Phase 3) are all implemented and merged, so `basis-producer` now consumes the gateway-side boundary Phase 1B built — though today only via a manually composed submission, not yet one driven by a real, adapter-normalized operation (Phase 4, below). §11's own repository-placement decision gate is now resolved by [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) — `basis-producer` is established as the permanent operation-producer-runtime repository and component, with Phase 1B's completed, proven gateway-side boundary as additional evidence that the trust boundary the producer sits behind is real rather than speculative; ADR-0010 is recorded `Accepted`. The `basis-foundation/basis-producer` repository now exists, and its first three implementation phases — Phase 2A (evidence-retention foundation), Phase 2B (reference lifecycle), and Phase 3 (authenticated gateway client) — are all implemented and merged. Phase 4 (REST adapter composition) and later phases remain unimplemented; the complete producer-to-gateway handoff, driven end to end from a real protocol operation, therefore remains incomplete.
...
Between those two, well-specified components sits a gap: **how a `NormalizedAuthorizationRequest` produced by `basis-adapters` actually becomes an authenticated, trusted-producer request accepted by `basis-gateway`.** At the time this document was originally authored, BASIS had not yet defined how that crossing would happen at all. Subsequent architecture work has resolved the bounded producer-authentication portion of that problem: [ADR-0007](../adr/0007-adapter-evidence-construction.md) established evidence-construction ownership, [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — Accepted — selected mTLS and gateway-owned producer-admission semantics, [`bounded-operation-producer-reference-implementation-plan.md`](bounded-operation-producer-reference-implementation-plan.md) defined a phased integration path, and [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — Accepted — now defines the trusted-ingress certificate-handoff topology required after that plan's Phase 1A reached Outcome B. Nothing in `basis-adapters` authenticates to the gateway. Nothing in `basis-gateway` accepts an unauthenticated adapter output directly. The two contracts are compatible in shape — the `AdapterEvidenceReference` model that `basis-gateway`'s operation-aware request accepts is structurally built to reference the evidence `basis-adapters` preserves. What remains is implementation: no component in the ecosystem today performs the complete handoff. The operation-producer runtime, `basis-producer`, now exists and has implemented evidence retention (Phase 2A), reference lifecycle (Phase 2B), and the authenticated gateway client (Phase 3); `ROADMAP.md` now names REST adapter composition (Phase 4) as the *next remaining integration boundary* — its authentication mechanism and reference-topology decisions are made, and evidence retention, reference lifecycle, and gateway submission are all implemented, but composing that submission from a real, adapter-normalized operation is not yet started.
...
- **The current gateway subject-ID allowlist is a valid, bounded first implementation — not the final ecosystem trust model.** `ROADMAP.md`, at the time this document was originally written, described `OPERATION_PRODUCER_SUBJECT_IDS` as "not the adapter-to-gateway trusted-producer contract... which remains open." This document itself did not close that gap; it named the shape of the decision (§13, Stage 4) without selecting a transport-authentication technology, per its own explicit non-goals at the time. **That gap is no longer open as an authentication-mechanism question.** [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md) — Accepted — selected mutual TLS as the first normative producer-to-gateway workload-authentication profile and retains the allowlist as a compatibility mechanism rather than removing it. What remains genuinely open, narrower than the original framing above, is no longer the bounded reference *implementation* of that already-selected mechanism at the gateway: `basis-gateway`'s Phase 1A spike found direct in-process mTLS termination unsafe (Outcome B), [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) — Accepted — defined the trusted-ingress topology required to carry a validated certificate into gateway application code, and `basis-gateway`'s Phase 1B (1B.1, 1B.2, and 1B.3) has since merged: gateway-side producer authentication and admission against that topology is implemented and proven. That work is now also implemented: the operation-producer runtime, `basis-producer`, now exists and has implemented evidence retention (Phase 2A), reference lifecycle (Phase 2B), and gateway submission (Phase 3). What remains genuinely open is composing that authenticated submission from a real, adapter-normalized operation rather than a manually assembled one (Phase 4). Category-scoped producer capability (the granularity question in the paragraph below) remains open independent of the authentication-mechanism question, and is not resolved by either ADR.
...
The governed handoff from normalization to gateway submission has four stages. Each stage is independently implemented today: `basis-adapters`' normalization and evidence-material surface (`RestAdapter.normalize()`, `protocol_evidence` construction) is implemented; `basis-producer`'s producer-identity binding and evidence-reference assembly mechanism (Phase 2A, Phase 2B) is implemented; `basis-producer`'s authenticated gateway submission (Phase 3) is implemented; and `basis-gateway`'s downstream operation-aware request handling into `basis-core` is implemented. What does not yet exist is a single live path chaining all four stages together, driven by a real adapter-normalized operation rather than a manually assembled submission — composing that path is Phase 4:

```text
ProtocolOperation
    ↓  (basis-adapters: implemented)
NormalizedAuthorizationRequest  (+ protocol_evidence, preserved verbatim)
    ↓  (operation-producer runtime: mechanism implemented — Phase 2A,
       Phase 2B, Phase 3; not yet driven by adapter-normalized
       output — Phase 4)
...
basis-producer
    ...
    management, producer workload credential custody, and authenticated
    gateway submission
    (established by ADR-0010, Accepted; repository exists, Phase 2A
    evidence-retention foundation, Phase 2B reference-lifecycle
    responsibility, and Phase 3 authenticated gateway client all
    implemented — the logical operation-producer role itself remains
    distinct from this named implementation; see §11)
...
5. **Bounded producer-runtime reference implementation** — a first, narrow implementation of the operation-producer runtime role (§2), scoped to one protocol and one deployment topology (§9), used to validate that the roles and rules this document defines are actually sufficient before generalizing them.
   **Decision gate:** does this reference implementation live in an existing repository (a `basis-gateway` client library, for instance) or does it justify a new repository, per §11's deferred decision? **Resolved:** [ADR-0010](../adr/0010-establish-basis-producer-as-operation-producer-runtime.md) establishes `basis-producer` as a new, permanent repository for this role; `Accepted`. The repository now exists, and its first three implementation phases — Phase 2A (evidence-retention foundation), Phase 2B (reference lifecycle), and Phase 3 (authenticated gateway client) — are all implemented and merged. **Implementation planning:** [`docs/architecture/bounded-operation-producer-reference-implementation-plan.md`](bounded-operation-producer-reference-implementation-plan.md), authorized by [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md), defines a phased implementation sequence for this reference implementation, now targeting the permanently established `basis-producer` repository per ADR-0010; Phase 2A, Phase 2B, and Phase 3 are implemented, Phase 4 (REST vertical slice) is the next implementation phase and has not yet begun. **Producer-mTLS transport-boundary sub-gate — resolved and implemented.** that plan's own Phase 1A named a further, narrower blocking gate — where mTLS actually terminates and how a validated client certificate's identity reaches gateway application code — which `basis-gateway`'s Phase 1A spike resolved to Outcome B (direct application-level termination is not a safe boundary under the current ASGI/Uvicorn stack), requiring a trusted mTLS ingress in front of the gateway per [ADR-0008](../adr/0008-producer-workload-authentication-and-admission.md)'s mTLS profile. [ADR-0009](../adr/0009-trusted-producer-mtls-ingress-and-gateway-certificate-handoff.md) and [`docs/architecture/producer-mtls-proxy-trust-boundary.md`](producer-mtls-proxy-trust-boundary.md) define that ingress topology and are recorded `Accepted`. `basis-gateway`'s Phase 1B — 1B.1, 1B.2, and 1B.3 — has since merged: gateway-side producer authentication and admission (trusted-ingress certificate handoff, URI SAN derivation, exact admission matching, and dual authentication of the producer workload independent of the bearer authorization subject) is implemented and proven for the bounded, approved scope this sub-gate describes. `basis-producer` now implements the evidence-retention, reference-lifecycle, and authenticated-gateway-client thirds of the work ADR-0007 and ADR-0008 assign to it (Phase 2A, Phase 2B, Phase 3) — the client holds a producer certificate and submits against this now-proven gateway boundary. What remains unimplemented is composing that client with real, adapter-normalized operation output (Phase 4). This sub-gate does not change §2's role definitions, §3's trust rules, or this document's own repository-placement decision gate — it narrowed only the transport question of how an already-decided authentication mechanism reaches gateway code, consistent with this document's own non-goal of not selecting a producer-authentication mechanism.
