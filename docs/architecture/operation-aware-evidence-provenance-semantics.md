...
**Status:** Approved and implemented. Architecture clarification, narrower than an ADR, opened to resolve three repeatable disagreements `basis-core`'s roadmap PR 37 ("Wire the five canonical scenarios end-to-end," `docs/implementation/basis-core-v0.2-operation-aware-plan.md`, Milestone 12, a document in the separate `basis-core` repository, not this one — referenced by path, not hyperlinked, per this repository's cross-repository citation convention) found between the merged `basis-core` implementation and the vendored `basis-schemas` v0.2.1 canonical compatibility fixtures. This clarification's governing semantics are approved and implemented: `basis-schemas` v0.2.2 corrected the vendored canonical compatibility fixtures to agree with it, and `basis-core` v0.2.0's canonical conformance suite passes against the corrected `v0.2.2` snapshot through the real `OperationAwareEnforcementPoint.evaluate()` path. This document settled the governing semantics; the fixture correction and `basis-core`'s rule-evidence projection alignment were the separate, later pull requests in their own repositories that this document anticipated, sequenced after this clarification per `basis-core`'s reconciliation runbook.
