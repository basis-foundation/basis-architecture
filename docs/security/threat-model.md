...
- [`docs/architecture/adapter-evidence-construction-semantics.md`](../architecture/adapter-evidence-construction-semantics.md) — extends the §3.3 trusted-adapter boundary and §6.3 adapter threats to the specific mechanics of adapter evidence material, canonicalization, and digest construction, with its own threat-by-threat analysis (evidence-material tampering, digest substitution, `reference_id` collision, replay, and more); does not close the adapter-to-gateway producer-authentication question named in §11 below
